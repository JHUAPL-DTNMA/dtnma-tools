...
def chunk_list(val: List, size: int) -> Generator:
    """Iterate a list in chunks"""
    for idx in range(0, len(val), size):
        yield val[idx : (idx + size)]


RPTT_CHUNK_SIZE = 40
""" Size of chunks for testing RPTT lists """
...
class TestRefdaSocket(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self._mgr_bind: List[BindInstance] = [bound_sock(f"mgr{index}.sock") for index in range(3)]
        # listen in work thread
        self._mgr_reader = threading.Thread(
            target=self._read_mgr, args=[self._mgr_bind], daemon=True
        )
        ...
        with open(startup_path, "w") as startup_file:
            startup_file.writelines(
                [
                    ...
                    "//ietf/dtnma-agent-acl/ctrl/ensure-group-members(1,/ac/(/label/0,//ietf/dtnma-agent/oper/match-regexp("
                    + mgr0_pat
                    + ")))\n",
                    "//ietf/dtnma-agent-acl/CTRL/ensure-access(1,/ac/(1),/ac/(true),/ac/("
                    + ",".join(
                        [
                    ...
                    "//ietf/dtnma-agent-acl/ctrl/ensure-group-members(2,/ac/(/label/0,//ietf/dtnma-agent/oper/match-regexp("
                    + mgr1_pat
                    + ")))\n",
                    "//ietf/dtnma-agent-acl/CTRL/ensure-access(2,/ac/(2),/ac/(/label/0,//ietf/dtnma-agent/oper/is-model-odm,//ietf/dtnma-agent/oper/bool-not),/ac/("
                    + ",".join(
                        [
                            "//ietf/dtnma-agent-acl/ident/execute",
                            "//ietf/dtnma-agent-acl/ident/produce",
    ...
    def _wait_reports(
        self, mgr_ix: int, nonce: Optional[ARI] = None, timeout: float = 1, stop_count: int = 1
    ) -> List[ari.Report]:
        ...
        while timer:
            LOGGER.debug(
                "waiting up to %s for %d reports, have %d already",
                timer.remaining(),
                stop_count,
                len(reports),
            )
    ...
    def test_exec_report_on_multi_mgr(self):
        ...
        self._send_msg(
            [
                self._ari_text_to_obj(
                    "ari:/EXECSET/n=123;("
                    + "//ietf/dtnma-agent/CTRL/report-on(//ietf/dtnma-agent/CONST/hello,/ac/("
                    + ",".join(mgr_eids)
                    + ")),"
                    + "//ietf/dtnma-agent/CTRL/report-on(//ietf/-1/const/1,/ac/("
                    + ",".join(mgr_eids)
                    + "))"
                    + ")"
                )
        ...
        rpt = rpts.pop(0)
        self.assertEqual(
            self._ari_text_to_obj("//ietf/dtnma-agent/ctrl/report-on"),
    ...
    def test_exec_report_on_no_destination(self):
        ...
        self.assertEqual(
            self._ari_text_to_obj(
                "//ietf/dtnma-agent/ctrl/report-on(//ietf/dtnma-agent/CONST/hello)"
            ),
            rpt.source,
        )
        # items of the report
    ...
    def test_exec_report_on_variations(self):
        ...
        self.assertEqual(
            self._ari_text_to_obj(
                "/ac/(//ietf/dtnma-agent/EDD/num-msg-rx,//ietf/dtnma-agent/EDD/num-msg-rx-failed)"
            ),
            rpt.source,
        )
        ...
        self.assertEqual(
            self._ari_text_to_obj(
                "/ac/(//ietf/dtnma-agent/EDD/num-msg-rx,//ietf/dtnma-agent/EDD/num-msg-rx-failed,//ietf/dtnma-agent/oper/add)"
            ),
            rpt.items[0],
        )
        ...
        rpt = rpts.pop(0)
        self.assertEqual(
            self._ari_text_to_obj("/ac/(/ac/(//ietf/!odm/const/expr-sum))"), rpt.source
        )
    ...
    def test_exec_delayed(self):
        ...
        rpt = rpts.pop(0)
        self.assertEqual(
            self._ari_text_to_obj("//ietf/dtnma-agent/CTRL/wait-for(/TD/PT1.5S)"), rpt.source
        )
        # items of the report
        self.assertEqual((ari.LiteralARI(None),), rpt.items)
    ...
    def test_exec_macro_failure_expand(self):
        ...
        rpt = rpts.pop(0)
        self.assertEqual(
            self._ari_text_to_obj("//ietf/dtnma-agent/ctrl/inspect(false)"), rpt.source
        )
    ...
    def test_exec_macro_failure_exec(self):
        ...
        rpt = rpts.pop(0)
        self.assertEqual(
            self._ari_text_to_obj("//ietf/dtnma-agent/ctrl/inspect(//!private/!odm/EDD/missing)"),
            rpt.source,
        )
        self.assertEqual((ari.UNDEFINED,), rpt.items)
    ...
    def test_exec_introspection(self):
        ...
        self._send_msg(
            [
                self._ari_text_to_obj(
                    "ari:/EXECSET/n=123;(//ietf/dtnma-agent/ctrl/inspect(//ietf/dtnma-agent/edd/exec-running))"
                )
            ]
        )
    ...
    def test_prod_variations(self):
        ...
        for rpt_ix, rpt in enumerate(rpts):
            ...
            self.assertNotIn(
                ari.UNDEFINED, rpt.items, msg=f"Failed on {self._ari_obj_to_text(rpt.source)}"
            )
        ...
        # no substitution
        self.assertEqual(
            self._ari_text_to_obj("/ac/(//ietf/dtnma-agent/EDD/num-msg-rx,/label/hello)"),
            rpt.items[0],
        )
        ...
        self.assertEqual(
            self._ari_text_to_obj("/ac/(//ietf/dtnma-agent/EDD/num-msg-rx,undefined)"), rpt.items[0]
        )
    ...
    def test_eval_strict_compare_opers(self):
        ...
        expect_sources: Dict[ARI, List[ARI]] = {}
        ...
        for left in cases:
            ...
            execset = self._ari_text_to_obj(
                "ari:/EXECSET/n=null;(//ietf/dtnma-agent/CTRL/report-on(/ac/("
                + ",".join(exprs)
                + ")))"
            )
    ...
    def test_eval_loose_compare_opers(self):
        ...
        cases = [
            ...
            # time points
            # FIXME ('10', '/tp/10', ari.TYPED_FALSE, ari.TYPED_TRUE, ari.TYPED_FALSE, ari.TYPED_FALSE, ari.TYPED_FALSE, ari.TYPED_FALSE),
            ("/td/10", "/tp/10", *([ari.UNDEFINED] * 6)),
            # time diffs
            # FIXME ('10', '/td/10', ari.TYPED_FALSE, ari.TYPED_TRUE, ari.TYPED_FALSE, ari.TYPED_FALSE, ari.TYPED_FALSE, ari.TYPED_FALSE),
        ...
        for row in cases:
            self.assertEqual(2 + len(oper_ref_col), len(row))

        self._start()

        for case_ix, oper_ref in enumerate(oper_ref_col):
            exprs = ["/ac/(" + row[0] + "," + row[1] + "," + oper_ref + ")" for row in cases]
    ...
    def test_eval_predicate_opers(self):
        ...
        cases = [
            ...
            (
                "//ietf/dtnma-agent/oper/ref(//ietf/-1/ident/3)",
                ari.TYPED_FALSE,
                ari.TYPED_TRUE,
                ari.TYPED_TRUE,
                ari.TYPED_FALSE,
            ...
            (
                "//ietf/dtnma-agent/oper/ref(//ietf/-1/)",
                ari.TYPED_FALSE,
                ari.TYPED_TRUE,
                ari.TYPED_TRUE,
                ari.TYPED_FALSE,
        ...
        for row in cases:
            self.assertEqual(1 + len(oper_ref_col), len(row))

        self._start()

        for case_ix, oper_ref in enumerate(oper_ref_col):
            exprs = ["/ac/(" + row[0] + "," + oper_ref + ")" for row in cases]
    ...
    def test_eval_type_convert_oper(self):
        ...
        cases = [
            ...
            (
                ...
                "/int/10",
                "/byte/10",
            ),  # does not match but can convert
    ...
    def test_eval_container_access_opers(self):
        # evaluate through report template items with inline expressions
        exprs, expect_items = map(
            list,
            zip(
                ...
                (
                    "/ac/(/ac/(1,2,3),//ietf/dtnma-agent/oper/list-get(10))",
                    ari.UNDEFINED,
                ),  # missing index
                (
                    "/ac/(/ac/(1,2,3),//ietf/dtnma-agent/oper/list-get(hi))",
                    ari.UNDEFINED,
                ),  # bad parameter
                ("/ac/(2,//ietf/dtnma-agent/oper/list-get(0))", ari.UNDEFINED),  # bad operand
                # map access
                (
                    "/ac/(/am/(1=one,2=two),//ietf/dtnma-agent/oper/map-get(1))",
                    ari.LiteralARI("one"),
                ),
                (
                    "/ac/(/am/(1=one,2=two),//ietf/dtnma-agent/oper/map-get(10))",
                    ari.UNDEFINED,
                ),  # missing key
                (
                    "/ac/(/am/(1=one,2=two),//ietf/dtnma-agent/oper/map-get(hi))",
                    ari.UNDEFINED,
                ),  # missing key but valid
                ("/ac/(2,//ietf/dtnma-agent/oper/map-get(0))", ari.UNDEFINED),  # bad operand
                # table access
                (
                    "/ac/(/tbl/c=2;(1,2)(3,4),//ietf/dtnma-agent/oper/tbl-get(1,0))",
                    ari.LiteralARI(3),
                ),
                (
                    "/ac/(/tbl/c=2;(1,2)(3,4),//ietf/dtnma-agent/oper/tbl-get(0,1))",
                    ari.LiteralARI(2),
                ),
                (
                    "/ac/(/tbl/c=2;(1,2)(3,4),//ietf/dtnma-agent/oper/tbl-get(10,0))",
                    ari.UNDEFINED,
                ),  # missing index
                (
                    "/ac/(/tbl/c=2;(1,2)(3,4),//ietf/dtnma-agent/oper/tbl-get(0,10))",
                    ari.UNDEFINED,
                ),  # missing index
                (
                    "/ac/(/tbl/c=2;(1,2)(3,4),//ietf/dtnma-agent/oper/tbl-get(hi,0))",
                    ari.UNDEFINED,
                ),  # bad parameter
                (
                    "/ac/(/tbl/c=2;(1,2)(3,4),//ietf/dtnma-agent/oper/tbl-get(0,hi))",
                    ari.UNDEFINED,
                ),  # bad parameter
    ...
    def test_eval_predicate_expressions(self):
        ...
        cases = [
            ...
            (
                "/ac/(//ietf/!odm/const/true-expr)",
                self._ari_text_to_obj("/ac/(2,2,//ietf/dtnma-agent/oper/compare-eq)"),
            ),
            ("/ac/(//ietf/dtnma-agent/oper/eval(//ietf/!odm/const/true-expr))", ari.TYPED_TRUE),
            (
                "/ac/(//ietf/dtnma-agent/oper/nary-eval(0,//ietf/!odm/const/true-expr))",
                ari.TYPED_TRUE,
            ),
            ("/ac/(//ietf/dtnma-agent/oper/nary-eval(0,/ac/(10)))", ari.LiteralARI(10)),
            (
                "/ac/(//ietf/dtnma-agent/oper/nary-eval(1,/ac/(/label/0)))",
                ari.UNDEFINED,
            ),  # fail from too small stack
            ("/ac/(10,//ietf/dtnma-agent/oper/nary-eval(1,/ac/(/label/0)))", ari.LiteralARI(10)),
            (
                "/ac/(10,2,//ietf/dtnma-agent/oper/nary-eval(2,/ac/(/label/0,/label/1,//ietf/dtnma-agent/oper/sub)))",
                ari.typed_int(8),
            ),
            (
                "/ac/(10,2,//ietf/dtnma-agent/oper/nary-eval(2,/ac/(/label/1,/label/0,//ietf/dtnma-agent/oper/sub)))",
                ari.typed_int(-8),
            ),
            (
                "/ac/(//ietf/dtnma-agent/oper/ref(//ietf/dtnma-agent/),//ietf/dtnma-agent/oper/is-model-odm)",
                ari.TYPED_FALSE,
            ),
            (
                "/ac/(//ietf/dtnma-agent/oper/ref(//ietf/-10/),//ietf/dtnma-agent/oper/is-model-odm)",
                ari.TYPED_TRUE,
            ),
            ...
            # verify is-same-ns
            (
                "/ac/(//ietf/dtnma-agent/oper/ref(//ietf/dtnma-agent/),//ietf/dtnma-agent/oper/is-same-ns(//ietf/dtnma-agent/))",
                ari.TYPED_TRUE,
            ),
            (
                "/ac/(//ietf/dtnma-agent/oper/ref(//ietf/dtnma-agent/const/hello),//ietf/dtnma-agent/oper/is-same-ns(//ietf/dtnma-agent/))",
                ari.TYPED_TRUE,
            ),
            (
                "/ac/(//ietf/dtnma-agent/oper/ref(//ietf/dtnma-agent-acl/),//ietf/dtnma-agent/oper/is-same-ns(//ietf/dtnma-agent/))",
                ari.TYPED_FALSE,
            ),
            (
                "/ac/(//ietf/dtnma-agent/oper/ref(//ietf/!missing/),//ietf/dtnma-agent/oper/is-same-ns(//ietf/dtnma-agent/))",
                ari.TYPED_FALSE,
            ),
            (
                "/ac/(//ietf/dtnma-agent/oper/ref(//ietf/!missing/),//ietf/dtnma-agent/oper/is-same-ns(//ietf/!missing/))",
                ari.UNDEFINED,
            ),  # no such instance
            # verify match-object-type
            (
                "/ac/(//ietf/dtnma-agent/oper/ref(//ietf/amm-base/typedef/integer),//ietf/dtnma-agent/oper/match-object-type(/aritype/typedef))",
                ari.TYPED_TRUE,
            ),
            (
                "/ac/(//ietf/dtnma-agent/oper/ref(//ietf/amm-base/typedef/integer),//ietf/dtnma-agent/oper/match-object-type(-12))",
                ari.TYPED_TRUE,
            ),
            (
                "/ac/(//ietf/dtnma-agent/oper/ref(//ietf/amm-base/typedef/integer),//ietf/dtnma-agent/oper/match-object-type(/aritype/const))",
                ari.TYPED_FALSE,
            ),
            (
                "/ac/(//ietf/dtnma-agent/oper/ref(//ietf/amm-base/typedef/integer),//ietf/dtnma-agent/oper/match-object-type(/aritype/uint))",
                ari.UNDEFINED,
            ),
            # verify match-object-int-range
            (
                "/ac/(//ietf/dtnma-agent/oper/ref(//ietf/amm-base/typedef/1),//ietf/dtnma-agent/oper/match-object-int-range(1,10))",
                ari.TYPED_TRUE,
            ),
            (
                "/ac/(//ietf/dtnma-agent/oper/ref(//ietf/amm-base/typedef/integer),//ietf/dtnma-agent/oper/match-object-int-range(1,10))",
                ari.TYPED_TRUE,
            ),
            (
                "/ac/(//ietf/dtnma-agent/oper/ref(//ietf/amm-base/typedef/11),//ietf/dtnma-agent/oper/match-object-int-range(1,10))",
                ari.TYPED_FALSE,
            ),
            (
                "/ac/(//ietf/dtnma-agent/oper/ref(//ietf/amm-base/typedef/counter32),//ietf/dtnma-agent/oper/match-object-int-range(1,10))",
                ari.TYPED_FALSE,
            ),
            # verify is-same-object
            (
                "/ac/(//ietf/dtnma-agent/oper/ref(//ietf/amm-base/typedef/integer),//ietf/dtnma-agent/oper/is-same-object(//ietf/amm-base/typedef/integer))",
                ari.TYPED_TRUE,
            ),
            (
                "/ac/(//ietf/dtnma-agent/oper/ref(//ietf/amm-base/typedef/numeric),//ietf/dtnma-agent/oper/is-same-object(//ietf/amm-base/typedef/integer))",
                ari.TYPED_FALSE,
            ),
            (
                "/ac/(//ietf/dtnma-agent/oper/ref(//ietf/!missing/typedef/obj),//ietf/dtnma-agent/oper/is-same-object(//ietf/amm-base/typedef/integer))",
                ari.TYPED_FALSE,
            ),
            (
                "/ac/(//ietf/dtnma-agent/oper/ref(//ietf/!missing/typedef/obj),//ietf/dtnma-agent/oper/is-same-object(//ietf/!missing/const/obj))",
                ari.UNDEFINED,
            ),  # no such instance
            ...
            (
                "/ac/(2,//ietf/dtnma-agent/oper/unary-eval(/ac/(/label/0,10,//ietf/dtnma-agent/oper/compare-eq)))",
                ari.TYPED_FALSE,
            ),
            (
                "/ac/(2,//ietf/dtnma-agent/oper/unary-eval(/ac/(/label/0,2,//ietf/dtnma-agent/oper/compare-eq)))",
                ari.TYPED_TRUE,
            ),
            (
                "/ac/(2,//ietf/dtnma-agent/oper/nary-eval(1,/ac/(/label/0,10,//ietf/dtnma-agent/oper/compare-eq)))",
                ari.TYPED_FALSE,
            ),
            (
                "/ac/(2,//ietf/dtnma-agent/oper/nary-eval(1,/ac/(/label/0,2,//ietf/dtnma-agent/oper/compare-eq)))",
                ari.TYPED_TRUE,
            ),
            (
                "/ac/(//ietf/dtnma-agent/oper/ref(//ietf/dtnma-agent/),//ietf/dtnma-agent/oper/unary-eval(/ac/(/label/0,//ietf/dtnma-agent/oper/is-model-odm)))",
                ari.TYPED_FALSE,
            ),
            (
                "/ac/(//ietf/dtnma-agent/oper/ref(//ietf/-10/),//ietf/dtnma-agent/oper/unary-eval(/ac/(/label/0,//ietf/dtnma-agent/oper/is-model-odm)))",
                ari.TYPED_TRUE,
            ),
            (
                "/ac/(//ietf/dtnma-agent/oper/ref(//ietf/dtnma-agent/),//ietf/dtnma-agent/oper/nary-eval(1,/ac/(/label/0,//ietf/dtnma-agent/oper/is-model-odm)))",
                ari.TYPED_FALSE,
            ),
            (
                "/ac/(//ietf/dtnma-agent/oper/ref(//ietf/-10/),//ietf/dtnma-agent/oper/nary-eval(1,/ac/(/label/0,//ietf/dtnma-agent/oper/is-model-odm)))",
                ari.TYPED_TRUE,
            ),
            # predicate composing operators
            ("/ac/(2,//ietf/dtnma-agent/oper/predicate-all(/ac/()))", ari.TYPED_FALSE),
            (
                "/ac/(2,//ietf/dtnma-agent/oper/predicate-all(/ac/(//ietf/dtnma-agent/oper/is-not-undefined)))",
                ari.TYPED_TRUE,
            ),
            (
                "/ac/(2,//ietf/dtnma-agent/oper/predicate-all(/ac/(//ietf/dtnma-agent/oper/is-undefined)))",
                ari.TYPED_FALSE,
            ),
            (
                "/ac/(2,//ietf/dtnma-agent/oper/predicate-all(/ac/(//ietf/dtnma-agent/oper/is-not-undefined,//ietf/dtnma-agent/oper/is-undefined)))",
                ari.TYPED_FALSE,
            ),
            ("/ac/(2,//ietf/dtnma-agent/oper/predicate-any(/ac/()))", ari.TYPED_FALSE),
            (
                "/ac/(2,//ietf/dtnma-agent/oper/predicate-any(/ac/(//ietf/dtnma-agent/oper/is-not-undefined)))",
                ari.TYPED_TRUE,
            ),
            (
                "/ac/(2,//ietf/dtnma-agent/oper/predicate-any(/ac/(//ietf/dtnma-agent/oper/is-undefined)))",
                ari.TYPED_FALSE,
            ),
            (
                "/ac/(2,//ietf/dtnma-agent/oper/predicate-any(/ac/(//ietf/dtnma-agent/oper/is-not-undefined,//ietf/dtnma-agent/oper/is-undefined)))",
                ari.TYPED_TRUE,
            ),
            ("/ac/(2,//ietf/dtnma-agent/oper/predicate-none(/ac/()))", ari.TYPED_TRUE),
            (
                "/ac/(2,//ietf/dtnma-agent/oper/predicate-none(/ac/(//ietf/dtnma-agent/oper/is-not-undefined)))",
                ari.TYPED_FALSE,
            ),
            (
                "/ac/(2,//ietf/dtnma-agent/oper/predicate-none(/ac/(//ietf/dtnma-agent/oper/is-undefined)))",
                ari.TYPED_TRUE,
            ),
            (
                "/ac/(2,//ietf/dtnma-agent/oper/predicate-none(/ac/(//ietf/dtnma-agent/oper/is-not-undefined,//ietf/dtnma-agent/oper/is-undefined)))",
                ari.TYPED_FALSE,
            ),
        ]
        for row in cases:
        ...
        # actual reporting in chunks
        for cases_chunk in chunk_list(cases, RPTT_CHUNK_SIZE):
            exprs, expect_items = map(list, zip(*cases_chunk))
            self._send_msg(
                [
                    self._ari_text_to_obj(
                        "ari:/EXECSET/n=null;(//ietf/dtnma-agent/CTRL/report-on(/ac/("
    ...
    def test_odm_control(self):
        ...
        rpt = rpts.pop(0)
        self.assertEqual(
            self._ari_text_to_obj("//ietf/dtnma-agent/CTRL/ensure-odm"),
            self._ari_strip_params(rpt.source),
        )
        # items of the report
        ...
        self.assertEqual(
            self._ari_text_to_obj(
                "//ietf/dtnma-agent/CTRL/inspect(//ietf/dtnma-agent/EDD/odm-list)"
            ),
            rpt.source,
        )
        # items of the report
        ...
        rpt = rpts.pop(0)
        self.assertEqual(
            self._ari_text_to_obj("//ietf/dtnma-agent/CTRL/ensure-tbr"),
            self._ari_strip_params(rpt.source),
        )
        # items of the report
        self.assertEqual([ari.NoneType], literal_prim_types(rpt.items))
        ...
        rpt = rpts.pop(0)
        self.assertEqual(
            self._ari_text_to_obj("//ietf/dtnma-agent/CTRL/ensure-sbr"),
            self._ari_strip_params(rpt.source),
        )
        # items of the report
        self.assertEqual([ari.NoneType], literal_prim_types(rpt.items))
        ...
        self.assertEqual(
            self._ari_text_to_obj(
                "//ietf/dtnma-agent/CTRL/inspect(//ietf/dtnma-agent/EDD/sbr-list(false))"
            ),
            rpt.source,
        )
        # items of the report
        ...
        self.assertEqual(
            self._ari_text_to_obj(
                "//ietf/dtnma-agent/CTRL/inspect(//ietf/dtnma-agent/EDD/tbr-list(false))"
            ),
            rpt.source,
        )
        # items of the report
        ...
        self._send_msg(
            [
                self._ari_text_to_obj(
                    "ari:/EXECSET/n=123;(//ietf/dtnma-agent/CTRL/ensure-rule-enabled(//ietf/!test-model-1/TBR/test-tbr,true))"
                )
            ]
        )
        rpts = self._wait_reports(mgr_ix=0, nonce=ari.LiteralARI(123))
        ...
        self.assertEqual(
            self._ari_text_to_obj(
                "//ietf/dtnma-agent/CTRL/ensure-rule-enabled(//ietf/!test-model-1/TBR/test-tbr,true)"
            ),
            rpt.source,
        )
        # items of the report
        ...
        self._send_msg(
            [
                self._ari_text_to_obj(
                    "ari:/EXECSET/n=123;(//ietf/dtnma-agent/CTRL/reset-rule-enabled(//ietf/!test-model-1/TBR/test-tbr))"
                )
            ]
        )
        rpts = self._wait_reports(mgr_ix=0, nonce=ari.LiteralARI(123))
        ...
        self.assertEqual(
            self._ari_text_to_obj(
                "//ietf/dtnma-agent/CTRL/reset-rule-enabled(//ietf/!test-model-1/TBR/test-tbr)"
            ),
            rpt.source,
        )
        # items of the report
        ...
        self._send_msg(
            [
                self._ari_text_to_obj(
                    "ari:/EXECSET/n=123;(//ietf/dtnma-agent/CTRL/obsolete-rule(//ietf/!test-model-1/TBR/test-tbr2))"
                )
            ]
        )
        rpts = self._wait_reports(mgr_ix=0, nonce=ari.LiteralARI(123))
        ...
        self.assertEqual(
            self._ari_text_to_obj(
                "//ietf/dtnma-agent/CTRL/obsolete-rule(//ietf/!test-model-1/TBR/test-tbr2)"
            ),
            rpt.source,
        )
        # items of the report
        ...
        self.assertEqual((1, 7), rpt.items[0].value.shape)
        self.assertNotIn(ari.UNDEFINED, list(rpt.items[0].value.flat))
        # Confirm rule is disabled
        self.assertEqual(False, rpt.items[0].value[0, 5].value)

        # obsolete the whole thing
        self._send_msg(
            [
                self._ari_text_to_obj(
                    "ari:/EXECSET/n=123;(//ietf/dtnma-agent/CTRL/obsolete-odm(//ietf/!test-model-1))"
                )
            ]
        )
        ...
        rpt = rpts.pop(0)
        self.assertEqual(
            self._ari_text_to_obj("//ietf/dtnma-agent/CTRL/obsolete-odm"),
            self._ari_strip_params(rpt.source),
        )
        # items of the report
        self.assertEqual([ari.NoneType], literal_prim_types(rpt.items))
    ...
    def test_rule_sbr(self):
        ...
        macro = (
            "/AC/(//ietf/dtnma-agent/CTRL/report-on("
            + rptsrc
            + ",/ac/("
            + ",".join(mgr_eids)
            + ")))"
        ...
        self._send_msg(
            [
                self._ari_text_to_obj(
                    "ari:/EXECSET/n=123;(//ietf/dtnma-agent/CTRL/ensure-sbr(//ietf/!test-model-1,test-sbr,3,"
                    + macro
                    + ","
                    + cond
                    + ",/TD/PT0.2S,0,true))"
        ...
        self._send_msg([self._ari_text_to_obj("ari:/EXECSET/n=null;()")])
    ...
    def test_edd_counters(self):
        ...
        self._send_msg(
            [
                self._ari_text_to_obj(
                    "ari:/EXECSET/n=123;(//ietf/dtnma-agent/CTRL/inspect(//ietf/dtnma-agent/EDD/num-exec-started))"
                )
            ]
        )
        rpts = self._wait_reports(mgr_ix=0, nonce=ari.LiteralARI(123))
        ...
        rpt = rpts.pop(0)
        self.assertEqual([int], literal_prim_types(rpt.items))
        # 4 from startup macro, 1 from <inspect> above
        ...
        self._send_msg(
            [
                self._ari_text_to_obj(
                    "ari:/EXECSET/n=123;(//ietf/dtnma-agent/CTRL/inspect(//ietf/dtnma-agent/EDD/num-exec-succeeded))"
                )
            ]
        )
        rpts = self._wait_reports(mgr_ix=0, nonce=ari.LiteralARI(123))
        ...
        self._send_msg(
            [
                self._ari_text_to_obj(
                    "ari:/EXECSET/n=123;(//ietf/dtnma-agent/CTRL/inspect(//ietf/dtnma-agent/EDD/num-exec-failed))"
                )
            ]
        )
        rpts = self._wait_reports(mgr_ix=0, nonce=ari.LiteralARI(123))
        ...
        self._send_msg(
            [
                self._ari_text_to_obj(
                    "ari:/EXECSET/n=123;(//ietf/dtnma-agent/CTRL/inspect(//ietf/dtnma-agent/EDD/num-msg-tx-failed))"
                )
            ]
        )
        rpts = self._wait_reports(mgr_ix=0, nonce=ari.LiteralARI(123))
        ...
        self._send_msg(
            [
                self._ari_text_to_obj(
                    "ari:/EXECSET/n=123;(//ietf/dtnma-agent/CTRL/inspect(//ietf/dtnma-agent/EDD/last-msg-rx-time))"
                )
            ]
        )
        rpts = self._wait_reports(mgr_ix=0, nonce=ari.LiteralARI(123))
    ...
    def test_odm_ident_valid(self):
        ...
        self._send_msg(
            [
                self._ari_text_to_obj(
                    "ari:/EXECSET/n=123;(//ietf/dtnma-agent/CTRL/inspect(//ietf/dtnma-agent/EDD/ident-list(false,false)))"
                )
            ]
        )
        rpts = self._wait_reports(mgr_ix=0, nonce=ari.LiteralARI(123))
        ...
        self._send_msg(
            [
                self._ari_text_to_obj(
                    "ari:/EXECSET/n=123;(//ietf/dtnma-agent/CTRL/inspect(//ietf/dtnma-agent/EDD/ident-list(false,true)))"
                )
            ]
        )
        rpts = self._wait_reports(mgr_ix=0, nonce=ari.LiteralARI(123))
        ...
        self._send_msg(
            [
                self._ari_text_to_obj(
                    "ari:/EXECSET/n=123;(//ietf/dtnma-agent/CTRL/ensure-ident(//ietf/!test-model-1,test-ident,1,false,/ac/(//ietf/alarms/IDENT/resource)))",
                    nn=False,
                )
            ]
        )
        ...
        self.assertNotIn(ari.UNDEFINED, list(rpt.items[0].value.flat))
        self.assertEqual(
            self._ari_text_to_obj("//1/-1/ident/1", nn=False), rpt.items[0].value[0, 0]
        )
        # Verify base-filtered list also shows object
        self._send_msg(
            [
                self._ari_text_to_obj(
                    "ari:/EXECSET/n=123;(//ietf/dtnma-agent/CTRL/inspect(//ietf/dtnma-agent/EDD/ident-list(false,false,//ietf/alarms/IDENT/resource)))"
                )
            ]
        )
        rpts = self._wait_reports(mgr_ix=0, nonce=ari.LiteralARI(123))
        ...
        self._send_msg(
            [
                self._ari_text_to_obj(
                    "ari:/EXECSET/n=123;(//ietf/dtnma-agent/CTRL/inspect(//ietf/dtnma-agent/EDD/ident-list(false,false,//ietf/alarms/IDENT/category)))"
                )
            ]
        )
        rpts = self._wait_reports(mgr_ix=0, nonce=ari.LiteralARI(123))
        ...
        self._send_msg(
            [
                self._ari_text_to_obj(
                    "ari:/EXECSET/n=123;(//ietf/dtnma-agent/CTRL/obsolete-ident(//ietf/!test-model-1/ident/test-ident))",
                    nn=False,
                )
            ]
        )
        ...
        self._send_msg(
            [
                self._ari_text_to_obj(
                    "ari:/EXECSET/n=123;(//ietf/dtnma-agent/CTRL/inspect(//ietf/dtnma-agent/EDD/ident-list(false)))"
                )
            ]
        )
        rpts = self._wait_reports(mgr_ix=0, nonce=ari.LiteralARI(123))
    ...
    def test_odm_var_valid(self):
        ...
        self._send_msg(
            [
                self._ari_text_to_obj(
                    "ari:/EXECSET/n=123;(//ietf/dtnma-agent/CTRL/inspect(//ietf/dtnma-agent/EDD/var-list(false)))"
                )
            ]
        )
        rpts = self._wait_reports(mgr_ix=0, nonce=ari.LiteralARI(123))
        ...
        self.assertEqual((1, 2), rpt.items[0].value.shape)
        self.assertNotIn(ari.UNDEFINED, list(rpt.items[0].value.flat))
        self.assertEqual(self._ari_text_to_obj("//1/-1/var/1"), rpt.items[0].value[0, 0])
        self.assertEqual(
            self._ari_text_to_obj(
                "//ietf/amm-semtype/IDENT/type-use(name=/ARITYPE/int)", id_convert=False
            ),
        ...
        self._send_msg(
            [
                self._ari_text_to_obj(
                    "ari:/EXECSET/n=123;(//ietf/dtnma-agent/CTRL/obsolete-var(//ietf/!test-model-1/VAR/test-var))"
                )
            ]
        )
        rpts = self._wait_reports(mgr_ix=0, nonce=ari.LiteralARI(123))
    ...
    def test_odm_const_valid(self):
        ...
        self._send_msg(
            [
                self._ari_text_to_obj(
                    "ari:/EXECSET/n=123;(//ietf/dtnma-agent/CTRL/inspect(//ietf/dtnma-agent/EDD/const-list(false)))"
                )
            ]
        )
        rpts = self._wait_reports(mgr_ix=0, nonce=ari.LiteralARI(123))
        ...
        self.assertEqual((1, 2), rpt.items[0].value.shape)
        self.assertNotIn(ari.UNDEFINED, list(rpt.items[0].value.flat))
        self.assertEqual(self._ari_text_to_obj("//1/-1/const/1"), rpt.items[0].value[0, 0])
        self.assertEqual(
            self._ari_text_to_obj(
                "//ietf/amm-semtype/IDENT/type-use(name=/ARITYPE/int)", id_convert=False
            ),
        ...
        # Verify it produces the value
        self._send_msg(
            [
                self._ari_text_to_obj(
                    "ari:/EXECSET/n=123;(//ietf/dtnma-agent/CTRL/inspect(//ietf/!test-model-1/const/1))"
                )
            ]
        )
        rpts = self._wait_reports(mgr_ix=0, nonce=ari.LiteralARI(123))
        ...
        rpt = rpts.pop(0)
        self.assertEqual([int], literal_prim_types(rpt.items))
        self.assertEqual((ari.LiteralARI(1),), rpt.items)

        # Obsolete the CONST
        self._send_msg(
            [
                self._ari_text_to_obj(
                    "ari:/EXECSET/n=123;(//ietf/dtnma-agent/CTRL/obsolete-const(//ietf/!test-model-1/CONST/test-const))"
                )
            ]
        )
        rpts = self._wait_reports(mgr_ix=0, nonce=ari.LiteralARI(123))
    ...
    def test_exec_control_flow_if_then_else(self):
        ...
        rpt = rpts.pop(0)
        self.assertEqual(
            self._ari_text_to_obj("//ietf/dtnma-agent/CTRL/if-then-else"),
            self._ari_strip_params(rpt.source),
        )
        self.assertEqual(1, len(rpt.items))
    ...
    def test_exec_control_flow_catch(self):
        ...
        rpt = rpts.pop(0)
        self.assertEqual(
            self._ari_text_to_obj("//ietf/dtnma-agent/CTRL/catch"),
            self._ari_strip_params(rpt.source),
        )
        self.assertEqual([bool], literal_prim_types(rpt.items))
        self.assertIs(True, rpt.items[0].value)
        ...
        rpt = rpts.pop(0)
        self.assertEqual(
            self._ari_text_to_obj("//ietf/dtnma-agent/CTRL/inspect(//ietf/!odm/EDD/missing)"),
            rpt.source,
        )
        self.assertEqual((ari.UNDEFINED,), rpt.items)
        # catch result after try target
        ...
        rpt = rpts.pop(0)
        self.assertEqual(
            self._ari_text_to_obj("//ietf/dtnma-agent/CTRL/inspect(//ietf/!odm/EDD/another)"),
            rpt.source,
        )
        self.assertEqual((ari.UNDEFINED,), rpt.items)
    ...
    def test_exec_control_flow_exec_deadline(self):
        ...
        rpt = rpts.pop(0)
        self.assertEqual(
            self._ari_text_to_obj("//ietf/dtnma-agent/CTRL/exec-deadline"),
            self._ari_strip_params(rpt.source),
        )
        self.assertEqual([bool], literal_prim_types(rpt.items))
        self.assertIs(True, rpt.items[0].value)
        ...
        rpt = rpts.pop(0)
        self.assertEqual(
            self._ari_text_to_obj("//ietf/dtnma-agent/CTRL/wait-cond(/ac/(false))"), rpt.source
        )
        self.assertEqual((ari.UNDEFINED,), rpt.items)
        # result after target
        rpt = rpts.pop(0)
    ...
    def test_odm_const_rptt(self):
        ...
        self._send_msg(
            [
                self._ari_text_to_obj(
                    "ari:/EXECSET/n=123;(//ietf/dtnma-agent/CTRL/report-on(//ietf/!test-model-1/const/rptt-num-msg-rx))"
                )
            ]
        )
    ...
    def test_acl_modify(self):
        ...
        self._send_msg(
            [
                self._ari_text_to_obj(
                    ...
                    + "//ietf/dtnma-agent-acl/ctrl/ensure-group-members(10,/ac/(/label/0,//ietf/dtnma-agent/oper/match-regexp("
                    + pat
                    + "))),"
                    + "//ietf/dtnma-agent-acl/CTRL/ensure-access(10,/ac/(10),/ac/(true),/ac/("
                    + ",".join(
                        [
                            "//ietf/dtnma-agent-acl/ident/execute",
                            "//ietf/dtnma-agent-acl/ident/produce",
        ...
        rpt = rpts.pop(0)
        self.assertEqual(
            self._ari_text_to_obj("//ietf/dtnma-agent-acl/CTRL/ensure-group"),
            self._ari_strip_params(rpt.source),
        )
        self.assertNotIn(ari.UNDEFINED, rpt.items)
        ...
        rpt = rpts.pop(0)
        self.assertEqual(
            self._ari_text_to_obj("//ietf/dtnma-agent-acl/CTRL/ensure-access"),
            self._ari_strip_params(rpt.source),
        )
        self.assertNotIn(ari.UNDEFINED, rpt.items)
        ...
        # new group number 10
        self.assertEqual(
            (ari.typed_uint(1), ari.typed_uint(2), ari.typed_uint(10)), rpt.items[0].value[:, 0]
        )
        # column 1: name
    ...
    def test_alarms(self):
        ...
        rpt = rpts.pop(0)
        self.assertEqual(
            self._ari_text_to_obj("//ietf/dtnma-agent/ctrl/inspect(//ietf/alarms/EDD/alarm-list)"),
            rpt.source,
        )
        self.assertEqual([ari.Table], literal_prim_types(rpt.items))
        ...
        rpt = rpts.pop(0)
        self.assertEqual(
            self._ari_text_to_obj("//ietf/dtnma-agent/ctrl/inspect(//ietf/alarms/EDD/shelf-list)"),
            rpt.source,
        )
        self.assertEqual([ari.Table], literal_prim_types(rpt.items))
        self.assertEqual((0, 2), rpt.items[0].value.shape)
    ...
    def test_alarms_shelf_manage(self):
        ...
        rpt = rpts.pop(0)
        self.assertEqual(
            self._ari_text_to_obj("//ietf/alarms/ctrl/ensure-shelf"),
            self._ari_strip_params(rpt.source),
        )
        self.assertNotIn(ari.UNDEFINED, rpt.items)
        ...
        rpt = rpts.pop(0)
        self.assertEqual(
            self._ari_text_to_obj("//ietf/alarms/ctrl/discard-shelf"),
            self._ari_strip_params(rpt.source),
        )
        self.assertNotIn(ari.UNDEFINED, rpt.items)
