...
class TestRefdaSocket(unittest.TestCase):
    ...
    def test_exec_report_on_variations(self):
        ...
        rpt = rpts.pop(0)
        self.assertEqual(self._ari_text_to_obj('//ietf/!odm/var/rptt-sum'), rpt.source)
        self.assertEqual(1, len(rpt.items))
        self.assertIsInstance(rpt.items[0].value, int)
    ...
    def test_prod_variations(self):
        ...
        self._send_msg(
            [self._ari_text_to_obj(
                ...
                + '//ietf/dtnma-agent/CTRL/ensure-const(' + (
                    ...
                    '),')
                + '//ietf/dtnma-agent/CTRL/ensure-const(' + (
                    '//ietf/!odm,expr-with-params,2,'  # identity
                    + '//ietf/amm-semtype/IDENT/type-use(//ietf/amm-base/typedef/expr),'  # type
                    + '/ac/(//ietf/dtnma-agent/EDD/num-msg-rx,/label/hello),'  # value
        ...
        self._send_msg(
            [self._ari_text_to_obj(
                ...
                + '//ietf/dtnma-agent/CTRL/inspect(//ietf/!odm/const/expr-with-params),'
                + '//ietf/dtnma-agent/CTRL/inspect(//ietf/!odm/const/expr-with-params(//ietf/dtnma-agent/EDD/num-msg-rx-failed))'
                + ')'
            )]
        )
        ...
        rpt = rpts.pop(0)
        self.assertEqual(
            self._ari_text_to_obj('//ietf/dtnma-agent/CTRL/inspect(//ietf/!odm/const/expr-with-params)'),
            rpt.source
        )
        self.assertEqual(1, len(rpt.items))
        ...
        rpt = rpts.pop(0)
        self.assertEqual(
            self._ari_text_to_obj('//ietf/dtnma-agent/CTRL/inspect(//ietf/!odm/const/expr-with-params(//ietf/dtnma-agent/EDD/num-msg-rx-failed))'),
            rpt.source
        )
        self.assertEqual(1, len(rpt.items))
        # substitution
    ...
    def test_odm(self):
        ...
        self._send_msg(
            [self._ari_text_to_obj(
                'ari:/EXECSET/n=123;(//ietf/dtnma-agent/CTRL/ensure-tbr(//ietf/!test-model-1,test-tbr,1,/AC/(//ietf/dtnma-agent/CTRL/obsolete-rule),/TD/999999,/TD/1,1,false))')]
        )
        rpts = self._wait_reports(mgr_ix=0, nonce=ari.LiteralARI(123))
        self.assertEqual(1, len(rpts))
        rpt = rpts.pop(0)
        ...
        self._send_msg(
            [self._ari_text_to_obj(
                'ari:/EXECSET/n=123;(//ietf/dtnma-agent/CTRL/ensure-tbr(//ietf/!test-model-1,test-tbr2,2,/AC/(//ietf/dtnma-agent/CTRL/obsolete-rule),/TD/999999,/TD/1,1,false))')]
        )
        rpts = self._wait_reports(mgr_ix=0, nonce=ari.LiteralARI(123))
        self.assertEqual(1, len(rpts))
        rpt = rpts.pop(0)
