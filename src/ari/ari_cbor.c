...
int ari_cbor_encode(ari_data_t *buf, const ari_t *ari)
{
    CHKERR1(buf);
    CHKERR1(ari);
    ...
    ari_data_copy_from(buf, needlen, NULL);
...
static int ari_cbor_decode_primval(QCBORDecodeContext *dec, ari_lit_t *lit)
{
    ...
    switch (decitem.uDataType)
    {
        ...
        case QCBOR_TYPE_TEXT_STRING:
            ...
            ari_data_init(&(lit->value.as_data));
            ari_data_copy_from(&(lit->value.as_data), decitem.val.string.len, (ari_data_ptr_t)decitem.val.string.ptr);
            ari_data_append_byte(&(lit->value.as_data), 0);
            break;
...
int ari_cbor_decode(ari_t *ari, const ari_data_t *buf, size_t *used, const char **errm)
{
    CHKERR1(ari);
    CHKERR1(buf);
