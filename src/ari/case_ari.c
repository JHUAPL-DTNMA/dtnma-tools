...
static int read_cbor(ari_t *inval, FILE *source)
{
    ...
    static ari_data_t store = ARI_DATA_INIT_NULL;
    ...
    while (true)
    {
        if (store.len < CBOR_STORE_WANT)
        {
            ...
            ari_data_append_from(&store, got, buf);
        }
    ...
    if (res && (res != QCBOR_ERR_EXTRA_BYTES))
    {
      ...
      return 2;
        ...
        if (used)
        {
            ari_data_extend_front(&store, -used);
        }
