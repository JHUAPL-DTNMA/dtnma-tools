%option reentrant bison-bridge noyywrap
%option nodefault
%option noinput nounput
%option noyyalloc noyyfree noyyrealloc
%option prefix="ari_val_"
%option extra-type="ari_text_val_t *"

%{
#include "ari_text_val_parse.h"
#include "ari_text_util.h"
#include "ari_config.h"
#include "util.h"
#include <m-string.h>

struct yyguts_t;
/** Use the extra data to set the initial state.
 */
static void ari_val_startcond(yyscan_t *scanner);
#define YY_USER_INIT ari_val_startcond(&yyscanner)
%}

%x PRIMITIVE
%x LT_NULL
%x LT_BOOL
%x LT_ANYINT
%x LT_ANYFLOAT
%x LT_TEXTSTR
%x LT_BYTESTR
%x LT_TIMEPOINT
%x LT_TIMEDIFF
%x LT_LABEL
%x LT_CBOR
%x LT_ARITYPE

HEXDIG [0-9a-fA-F]
DECDIG [0-9]

/* Tokens named similarly to Python anms-ace:ace.ari_text.util */
%%

<PRIMITIVE>(?i:undefined) {
    yylval->lit = (ari_lit_t){
        .prim_type = ARI_PRIM_UNDEFINED,
    };
    return T_UNDEFINED;
}
<PRIMITIVE,LT_NULL>(?i:null) {
    yylval->lit = (ari_lit_t){
        .prim_type = ARI_PRIM_NULL,
    };
    return T_NULL;
}
<PRIMITIVE,LT_BOOL>(?i:false) {
    yylval->lit = (ari_lit_t){
        .prim_type = ARI_PRIM_BOOL,
        .value.as_bool = false,
    };
    return T_BOOL;
}
<PRIMITIVE,LT_BOOL>(?i:true) {
    yylval->lit = (ari_lit_t){
        .prim_type = ARI_PRIM_BOOL,
        .value.as_bool = true,
    };
    return T_BOOL;
}

<PRIMITIVE,LT_ANYINT,LT_ARITYPE>[+-]?(0[bB][01]+|0[xX]{HEXDIG}+|{DECDIG}+) {
    // infer the base from the text
    if (yytext[0] == '-')
    {
        string_t text;
        string_init_set_str(text, yytext + 1);
        uint64_t neg;
        int ret = ari_uint64_decode(&neg, text);
        string_clear(text);
        if (ret)
        {
            ari_val_error(yyscanner, yyextra, "number too large to parse");
            return YYerror;
        }

        // work around signed negation overflow
        if (neg > (uint64_t)(-(INT64_MIN + 1)) + 1)
        {
            ari_val_error(yyscanner, yyextra, "negative int too large");
            return YYerror;
        }

        yylval->lit = (ari_lit_t){
            .prim_type = ARI_PRIM_INT64,
            .value.as_int64 = -(int64_t)neg,
        };
        return T_INT;
    }
    else
    {
        string_t text;
        string_init_set_str(text, yytext);
        uint64_t val;
        int ret = ari_uint64_decode(&val, text);
        string_clear(text);
        if (ret)
        {
            ari_val_error(yyscanner, yyextra, "number too large to parse");
            return YYerror;
        }

        // prefer signed integer use
        if (val <= INT64_MAX)
        {
            yylval->lit = (ari_lit_t){
                .prim_type = ARI_PRIM_INT64,
                .value.as_int64 = (int64_t)val,
            };
            return T_INT;
        }
        else
        {
            yylval->lit = (ari_lit_t){
                .prim_type = ARI_PRIM_UINT64,
                .value.as_uint64 = val,
            };
            return T_UINT;
        }
    }
}

<PRIMITIVE,LT_ANYFLOAT>[+-]?(({DECDIG}+|{DECDIG}*\.{DECDIG}*)([eE][+-]?{DECDIG}+)|0[xX]{HEXDIG}\.{HEXDIG}*([pP][+-]?{DECDIG})|{DECDIG}*(\.{DECDIG}*)?|Infinity)|NaN {
    yylval->lit = (ari_lit_t){
        .prim_type = ARI_PRIM_FLOAT64,
        .value.as_float64 = strtod(yytext, NULL),
    };
    return T_FLOAT;
}

<PRIMITIVE,LT_TEXTSTR,LT_LABEL>\"(\\.|[^"])*\" {
    char *text_begin = yytext + 1;
    char *text_end = yytext + yyleng - 1;
    const size_t text_len = text_end - text_begin;
    *text_end = '\0';

    ari_data_t data;
    ari_data_init(&data);
    if (text_len > 0)
    {
        ari_data_t text_view;
        ari_data_init_view(&text_view, text_len, (ari_data_ptr_t)text_begin);

        string_t text_unesc;
        string_init(text_unesc);
        slash_unescape(text_unesc, &text_view);
        ari_data_deinit(&text_view);

        ari_data_from_m_string(&data, text_unesc);
        string_clear(text_unesc);
    }

    yylval->lit = (ari_lit_t){
        .prim_type = ARI_PRIM_TSTR,
        .value.as_data = data,
    };
    return T_TSTR;
}

<PRIMITIVE,LT_BYTESTR,LT_CBOR>(h|b64)?'((\\.|[^'])*)' {
    char *form_begin = yytext;
    char *form_end = strchr(yytext, '\'');
    *form_end = '\0';

    char *text_begin = form_end + 1;
    char *text_end = yytext + yyleng - 1;
    const size_t text_len = text_end - text_begin;
    *text_end = '\0';

    ari_data_t data;
    ari_data_init(&data);
    if (text_len > 0)
    {
        if (strcmp(form_begin, "h") == 0)
        {
            string_t clean;
            string_init(clean);
            strip_space(clean, text_begin, text_len);
            int ret = base16_decode(&data, clean);
            string_clear(clean);
            if (ret)
            {
                ari_val_error(yyscanner, yyextra, "base16 failed to decode");
                return YYerror;
            }
        }
        else if (strcmp(form_begin, "b64") == 0)
        {
            string_t clean;
            string_init(clean);
            strip_space(clean, text_begin, text_len);
            int ret = base64_decode(&data, clean);
            string_clear(clean);
            if (ret)
            {
                ari_val_error(yyscanner, yyextra, "base64 failed to decode");
                return YYerror;
            }
        }
        else
        {
            ari_data_t text_view;
            ari_data_init_view(&text_view, text_len, (ari_data_ptr_t)text_begin);

            string_t text_unesc;
            string_init(text_unesc);
            slash_unescape(text_unesc, &text_view);
            ari_data_deinit(&text_view);
        
            // direct copy
            ari_data_from_m_string(&data, text_unesc);
            string_clear(text_unesc);
            // strip the trailing null
            ari_data_extend_back(&data, -1);
        }
    }

    yylval->lit = (ari_lit_t){
        .prim_type = ARI_PRIM_BSTR,
        .value.as_data = data,
    };
    return T_BSTR;
}

<LT_TIMEPOINT>({DECDIG}{4})\-?({DECDIG}{2})\-?({DECDIG}{2})T({DECDIG}{2}):?({DECDIG}{2}):?({DECDIG}{2})(\.({DECDIG}{1,6}))?Z {
    ari_data_t text_view;
    ari_data_init_view(&text_view, yyleng, (ari_data_ptr_t)yytext);

    struct timespec val;
    int ret = utctime_decode(&val, &text_view);
    ari_data_deinit(&text_view);
    if (ret)
    {
        return YYerror;
    }

    yylval->lit = (ari_lit_t){
        .prim_type = ARI_PRIM_TIMESPEC,
        .value.as_timespec = val,
    };
    return T_TIMEPOINT;
}

<LT_TIMEDIFF>([+-])?P(({DECDIG}+)D)?T(({DECDIG}+)H)?(({DECDIG}+)M)?(({DECDIG}+)(\.({DECDIG}{1,6}))?S)? {
    ari_data_t text_view;
    ari_data_init_view(&text_view, yyleng, (ari_data_ptr_t)yytext);

    struct timespec val;
    int ret = timeperiod_decode(&val, &text_view);
    ari_data_deinit(&text_view);
    if (ret)
    {
        return YYerror;
    }

    yylval->lit = (ari_lit_t){
        .prim_type = ARI_PRIM_TIMESPEC,
        .value.as_timespec = val,
    };
    return T_TIMEDIFF;
}

<LT_TIMEPOINT,LT_TIMEDIFF>([+-])?{DECDIG}*(\.{DECDIG}*)? {
    ari_data_t text_view;
    ari_data_init_view(&text_view, yyleng, (ari_data_ptr_t)yytext);
    
    struct timespec val;
    int ret = decfrac_decode(&val, &text_view);
    ari_data_deinit(&text_view);
    if (ret)
    {
        return YYerror;
    }

    yylval->lit = (ari_lit_t){
        .prim_type = ARI_PRIM_TIMESPEC,
        .value.as_timespec = val,
    };
    return T_DECFRAC;
}

<PRIMITIVE,LT_TEXTSTR,LT_LABEL,LT_ARITYPE>[a-zA-Z_][a-zA-Z0-9_\-\.]* {
    // Special case of unquoted text
    ari_data_t data;
    ari_data_init(&data);
    ari_data_copy_from(&data, yyleng + 1, (ari_data_ptr_t)yytext);

    yylval->lit = (ari_lit_t){
        .prim_type = ARI_PRIM_TSTR,
        .value.as_data = data,
    };
    return T_IDENTITY;
}

<LT_CBOR>\<\<(.*)\>\> {
    return T_CBOR_DIAG;
}

<*>\0|\n {
    // ignore trailing null
}

<*>. {
    string_t str;
    string_init(str);
    string_cat_printf(str, "Unexpected character at %d: %s (%d)",
                      yycolumn, yytext, yytext[0]);

    ari_val_error(yyscanner, yyextra, string_get_cstr(str));
    string_clear(str);
    return YYerror;
}

%%

void *ari_val_alloc(yy_size_t size, yyscan_t yyscanner _U_)
{
    return ARI_MALLOC(size);
}

void *ari_val_realloc(void *ptr, yy_size_t size, yyscan_t yyscanner _U_)
{
    return ARI_REALLOC(ptr, size);
}

void ari_val_free(void *ptr, yyscan_t yyscanner _U_)
{
    ARI_FREE(ptr);
}

void ari_val_startcond(yyscan_t *scanner)
{
    const ari_text_val_t *input = ari_val_get_extra(*scanner);
    const int32_t *ari_type = input->lit->has_ari_type ? &(input->lit->ari_type) : NULL;

    struct yyguts_t *yyg = (struct yyguts_t*)*scanner;
//    fprintf(stderr, "Context ari_type_t: %d\n", ari_type ? *ari_type : -1);
    if (!ari_type)
    {
        BEGIN(PRIMITIVE);
        return;
    }
    switch (*ari_type)
    {
        case ARI_TYPE_NULL:
            BEGIN(LT_NULL);
            break;
        case ARI_TYPE_BOOL:
            BEGIN(LT_BOOL);
            break;
        case ARI_TYPE_BYTE:
        case ARI_TYPE_UINT:
        case ARI_TYPE_UVAST:
            BEGIN(LT_ANYINT);
            break;
        case ARI_TYPE_INT:
        case ARI_TYPE_VAST:
            BEGIN(LT_ANYINT);
            break;
        case ARI_TYPE_REAL32:
        case ARI_TYPE_REAL64:
            BEGIN(LT_ANYFLOAT);
            break;
        case ARI_TYPE_TEXTSTR:
            BEGIN(LT_TEXTSTR);
            break;
        case ARI_TYPE_BYTESTR:
            BEGIN(LT_BYTESTR);
            break;

        case ARI_TYPE_TP:
            BEGIN(LT_TIMEPOINT);
            break;
        case ARI_TYPE_TD:
            BEGIN(LT_TIMEDIFF);
            break;
        case ARI_TYPE_LABEL:
            BEGIN(LT_LABEL);
            break;
        case ARI_TYPE_CBOR:
            BEGIN(LT_CBOR);
            break;
        case ARI_TYPE_ARITYPE:
            BEGIN(LT_ARITYPE);
            break;

        default:
            break;
    }
}
