...
%option prefix="ari_str_"
%option extra-type="ari_text_str_t *"
...
<INITIAL>\/\/{VALSEG} {
...

    {
        ...
        {
            ari_str_error(yyscanner, yyextra, "NS-ID segment failed to percent-decode");
            return YYerror;
        }
...
<ARI_OBJREF>\/{VALSEG} {
...

    {
        ...
        {
            ari_str_error(yyscanner, yyextra, "ID segment failed to percent-decode");
            return YYerror;
        }
...
<INITIAL>\/{VALSEG}\//. {
    ...
    ari_idseg_t typeid;
    {
        ...
        {
            ari_str_error(yyscanner, yyextra, "TYPE-ID segment failed to percent-decode");
            return YYerror;
        }
    ...
    case ARI_IDSEG_TEXT:
    {
        ...
        if (res)
        {
            ...
            ari_str_error(yyscanner, yyextra, "TYPE-ID has invalid name");
            return YYerror;
        }
        break;
    }
...
//    fprintf(stderr, "VALSEG from: %s\n", yytext);
    ...
    if (text_len > 0)
    {
        ...
        ari_data_init_view(&text_view, text_len + 1, (ari_data_ptr_t)text_begin);
...
void *ari_str_alloc(yy_size_t size, yyscan_t yyscanner _U_)
{
    return ARI_MALLOC(size);
}

void *ari_str_realloc(void *ptr, yy_size_t size, yyscan_t yyscanner _U_)
{
    return ARI_REALLOC(ptr, size);
}

void ari_str_free(void *ptr, yyscan_t yyscanner _U_)
{
    ARI_FREE(ptr);
}
...
{
    ari_data_t text_view;
    ari_data_init_view(&text_view, inlen + 1, (ari_data_ptr_t)in);
