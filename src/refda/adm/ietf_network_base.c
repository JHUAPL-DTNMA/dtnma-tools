...
int refda_adm_ietf_network_base_init(refda_agent_t *agent)
{
    ...
    cace_amm_obj_ns_t *adm = cace_amm_obj_store_add_ns(
        &(agent->objs),
        cace_amm_idseg_ref_withenum(REFDA_ADM_IETF_NETWORK_BASE_ORG_NAME, REFDA_ADM_IETF_NETWORK_BASE_ORG_ENUM),
        cace_amm_idseg_ref_withenum(REFDA_ADM_IETF_NETWORK_BASE_MODEL_NAME, REFDA_ADM_IETF_NETWORK_BASE_MODEL_ENUM),
        REFDA_ADM_IETF_NETWORK_BASE_MODEL_REVISION);
    if (adm)
    {
