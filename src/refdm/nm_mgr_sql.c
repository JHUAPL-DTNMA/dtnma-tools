 ...
 **/
void db_logf_msg(size_t dbidx, const char *msg, const char *details, int level, const char *fun, const char *file,
                 size_t line, ...)
{
    va_list args;
    va_start(args, line);
...
void db_log_msg(size_t dbidx, const char *msg, const char *details, int level, const char *fun, const char *file,
                size_t line)
{
    amp_log(level, 'd', file, line, fun, "%s \t %s", msg, ((details == NULL) ? "" : details));
...
// Initialize prepared queries
            ...
            queries[idx][AC_INSERT] = db_mgr_sql_prepare(
                idx, "SELECT insert_ac_actual_entry(?,?, ?)"); // ac_id, obj_actual_definition_id, idx

            queries[idx][AC_GET] =
                db_mgr_sql_prepare(idx, "SELECT ace.obj_actual_definition_id "
            ...
            queries[idx][ARI_GET] =
                db_mgr_sql_prepare(idx, "SELECT data_type_id, adm_type, adm_enum, obj_enum, tnvc_id, issuing_org FROM "
                                        "vw_ari WHERE obj_actual_definition_id=?");
            ...
            queries[idx][ARI_INSERT_CTRL] = db_mgr_sql_prepare(
                idx, "SELECT insert_ari_ctrl(?,?,NULL)"); // obj_metadata_id, actual_parmspec_id, description

            queries[idx][TNVC_CREATE] = db_mgr_sql_prepare(idx, "SELECT create_tnvc(NULL)"); // use_desc
            queries[idx][TNVC_INSERT_AC] =
                db_mgr_sql_prepare(idx, "SELECT insert_tnvc_ac_entry(?,NULL,NULL,?) "); // tnvc_id, ac_id
            queries[idx][TNVC_INSERT_ARI] =
                db_mgr_sql_prepare(idx, "SELECT insert_tnvc_obj_entry(?,NULL,NULL,?) "); // tnvc_id, obj_id
            queries[idx][TNVC_INSERT_TNVC] =
                db_mgr_sql_prepare(idx, "SELECT insert_tnvc_tnvc_entry(?,NULL,NULL,?) "); // tnvc_id, tnvc_id

            queries[idx][TNVC_INSERT_STR] =
                db_mgr_sql_prepare(idx, "SELECT insert_tnvc_str_entry(?,NULL,NULL,?) "); // tnvc_id, ac_id
            queries[idx][TNVC_INSERT_BOOL] =
                db_mgr_sql_prepare(idx, "SELECT insert_tnvc_bool_entry(?,NULL,NULL,?) "); // tnvc_id, ac_id
            queries[idx][TNVC_INSERT_BYTE] =
                db_mgr_sql_prepare(idx, "SELECT insert_tnvc_byte_entry(?,NULL,NULL,?) "); // tnvc_id, ac_id
            queries[idx][TNVC_INSERT_UINT] =
                db_mgr_sql_prepare(idx, "SELECT insert_tnvc_uint_entry(?,NULL,NULL,?) "); // tnvc_id, ac_id
            queries[idx][TNVC_INSERT_VAST] =
                db_mgr_sql_prepare(idx, "SELECT insert_tnvc_vast_entry(?,NULL,NULL,?) "); // tnvc_id, ac_id
            queries[idx][TNVC_INSERT_TV] =
                db_mgr_sql_prepare(idx, "SELECT insert_tnvc_tv_entry(?,NULL,NULL,?) "); // tnvc_id, ac_id
            queries[idx][TNVC_INSERT_TS] =
                db_mgr_sql_prepare(idx, "SELECT insert_tnvc_ts_entry(?,NULL,NULL,?) "); // tnvc_id, ac_id
            queries[idx][TNVC_INSERT_UVAST] =
                db_mgr_sql_prepare(idx, "SELECT insert_tnvc_uvast_entry(?,NULL,NULL,?) "); // tnvc_id, ac_id
            queries[idx][TNVC_INSERT_REAL32] =
                db_mgr_sql_prepare(idx, "SELECT insert_tnvc_real32_entry(?,NULL,NULL,?) "); // tnvc_id, ac_id
            queries[idx][TNVC_INSERT_REAL64] =
                db_mgr_sql_prepare(idx, "SELECT insert_tnvc_real64_entry(?,NULL,NULL,?) "); // tnvc_id, ac_id

            queries[idx][TNVC_VIEW] =
                db_mgr_sql_prepare(idx, "SELECT * FROM type_name_value_collection WHERE tnvc_id = ?");
            queries[idx][TNVC_ENTRIES] =
                db_mgr_sql_prepare(idx, "SELECT data_type_id, int_value, uint_value, obj_value, str_value, ac_value, "
                                        "tnvc_value FROM vw_tnvc_entries WHERE tnvc_id = ? ORDER BY order_num ASC");

            queries[idx][TNVC_PARMSPEC_INSERT_AC] =
                db_mgr_sql_prepare(idx, "CALL SP__insert_actual_parms_ac(?,?,?)"); // ap_spec_id, order_num, ac_id
            queries[idx][TNVC_PARMSPEC_INSERT_TNVC] =
                db_mgr_sql_prepare(idx, "CALL SP__insert_actual_parms_tnvc(?,?,?)"); // ap_spec_id, order_num, tnvc_id
            queries[idx][TNVC_PARMSPEC_CREATE] = db_mgr_sql_prepare(
                idx, "SELECT create_actual_parmspec_tnvc(?,?,NULL)"); // formal_def_id, tnvc_id, description

            queries[idx][MSGS_GET] = db_mgr_sql_prepare(idx, "SELECT * FROM message_group WHERE group_id=?");
            queries[idx][MSG_GET_AGENTS] =
                db_mgr_sql_prepare(idx, "SELECT agent_id_string FROM vw_message_agents WHERE message_id=?");
            queries[idx][MSGS_GET_AGENTS] =
                db_mgr_sql_prepare(idx, "SELECT agent_id_string FROM vw_message_group_agents WHERE group_id=?");
            queries[idx][MSGS_ENTRIES_GET] =
                db_mgr_sql_prepare(idx, "SELECT * FROM message_group_entry WHERE group_id=? ORDER BY order_num ASC");
            queries[idx][MSGS_ENTRIES_GET_AGENTS] =
                db_mgr_sql_prepare(idx, "SELECT * FROM vw_message_agents WHERE message_id=?");
            queries[idx][MSGS_UPDATE_GROUP_STATE] =
                db_mgr_sql_prepare(idx, "UPDATE message_group mg SET state_id=? WHERE group_id=?");
            queries[idx][MSGS_OUTGOING_GET] =
                db_mgr_sql_prepare(idx, "SELECT group_id, ts FROM vw_ready_outgoing_message_groups");
            queries[idx][MSGS_OUTGOING_CREATE] =
                db_mgr_sql_prepare(idx, "INSERT INTO message_group (state_id, is_outgoing) VALUES(1, TRUE)");

            // skywalker uses anms patch to fix these reported issues
            // queries[idx][MSGS_INCOMING_GET]    = db_mgr_sql_prepare(idx,"SELECT * FROM
            // vw_ready_INCOMING_message_groups"); queries[idx][MSGS_INCOMING_CREATE] = db_mgr_sql_prepare(idx,"SELECT
            // create_incoming_message_group(FROM_UNIXTIME(?), ? )"); // Received timestamp, From Agent name (ie:
            // ipn:2.1)
            queries[idx][MSGS_INCOMING_GET] = db_mgr_sql_prepare(idx, "SELECT * FROM vw_ready_incoming_message_groups");
            queries[idx][MSGS_INCOMING_CREATE] = db_mgr_sql_prepare(
                idx,
                "SELECT create_incoming_message_group(?, ? )"); // Received timestamp, From Agent name (ie: ipn:2.1)
            ...
            // skywalker uses anms patch to fix these reported issues
            // queries[idx][MSGS_ADD_REPORT_SET_ENTRY] = db_mgr_sql_prepare(idx,"SELECT insert_message_report_entry(?,
            // NULL, ?, ?, FROM_UNIXTIME(?))"); // message_id, order_num, ari_id, tnvc_id, ts
            queries[idx][MSGS_ADD_REPORT_SET_ENTRY] = db_mgr_sql_prepare(
                idx,
                "SELECT insert_message_report_entry(?, NULL, ?, ?, ?)"); // message_id, order_num, ari_id, tnvc_id, ts

            queries[idx][MSGS_REGISTER_AGENT_INSERT] = db_mgr_sql_prepare(
                idx, "SELECT add_message_register_entry(?,?,?,?,NULL,?)"); // group_id, ack, nak, acl, idx, agent_name
            queries[idx][MSGS_REGISTER_AGENT_GET] =
                db_mgr_sql_prepare(idx, "SELECT * FROM message_agents WHERE message_id = ?");
            queries[idx][MSGS_PERF_CTRL_INSERT] = db_mgr_sql_prepare(
                idx, "SELECT add_message_ctrl_entry(?, ?, ?, ?, ?, ?, ?)"); // group_id, ack, nak, acl, idx, timevalue
                                                                            // or NULL, ac_id
            queries[idx][MSGS_PERF_CTRL_GET] =
                db_mgr_sql_prepare(idx, "SELECT tv, ac_id FROM message_perform_control WHERE message_id=?");
            queries[idx][MSGS_REPORT_SET_INSERT] =
                db_mgr_sql_prepare(idx, "SELECT add_message_report_set(?,?,?,?,NULL)"); // group_id, ack, nak, acl, idx
            queries[idx][MSGS_REPORT_SET_GET] = db_mgr_sql_prepare(
                idx, "SELECT * FROM report_template_actual_definition WHERE obj_actual_definition_id=?");

            // TODO MSGS_TABLE_SET_INSERT/GET

            queries[idx][DB_LOG_MSG] = db_mgr_sql_prepare(
                idx, "INSERT INTO nm_mgr_log (msg,details,level,source,file,line) VALUES(?,?,?,?,?,?)");
#endif // HAVE_MYSQL
#ifdef HAVE_POSTGRESQL
            queries[idx][AC_CREATE] = db_mgr_sql_prepare(idx, "SELECT create_ac($1::int4,$2::varchar)", "AC_CREATE", 2,
                                                         NULL); // num_entries, use_desc
            queries[idx][AC_INSERT] =
                db_mgr_sql_prepare(idx, "SELECT insert_ac_actual_entry($1::int4, $2::int4, $3::int4)", "AC_INSERT", 3,
                                   NULL); // ac_id, obj_actual_definition_id, idx

            queries[idx][AC_GET] =
                db_mgr_sql_prepare(idx,
                                   "SELECT ace.obj_actual_definition_id "
                                   "FROM ari_collection_entry ac "
                                   "LEFT JOIN ari_collection_actual_entry ace ON ace.ac_entry_id=ac.ac_entry_id "
            ...
            queries[idx][ARI_GET] =
                db_mgr_sql_prepare(idx,
                                   "SELECT data_type_id, adm_type, adm_enum, obj_enum, tnvc_id, issuing_org FROM "
            ...
            queries[idx][TNVC_CREATE] =
                db_mgr_sql_prepare(idx, "SELECT create_tnvc(NULL)", "TNVC_CREATE", 0, NULL); // use_desc
            queries[idx][TNVC_INSERT_AC] =
                db_mgr_sql_prepare(idx, "SELECT insert_tnvc_ac_entry($1::int4,NULL,NULL,$2::int4) ", "TNVC_INSERT_AC",
                                   2, NULL); // tnvc_id, integer val
            queries[idx][TNVC_INSERT_ARI] =
                db_mgr_sql_prepare(idx, "SELECT insert_tnvc_obj_entry($1::int4,NULL,NULL,$2::int4) ", "TNVC_INSERT_ARI",
                                   2, NULL); // tnvc_id, integer val
            queries[idx][TNVC_INSERT_TNVC] =
                db_mgr_sql_prepare(idx, "SELECT insert_tnvc_tnvc_entry($1::int4,NULL,NULL,$2::int4) ",
                                   "TNVC_INSERT_TNVC", 2, NULL); // tnvc_id, integer val

            queries[idx][TNVC_INSERT_STR] =
                db_mgr_sql_prepare(idx, "SELECT insert_tnvc_str_entry($1::int4,NULL,NULL,$2::varchar) ",
                                   "TNVC_INSERT_STR", 2, NULL); // tnvc_id, varchar val
            queries[idx][TNVC_INSERT_BOOL] =
                db_mgr_sql_prepare(idx, "SELECT insert_tnvc_bool_entry($1::int4,NULL,NULL,$2::boolean) ",
                                   "TNVC_INSERT_BOOL", 2, NULL); // tnvc_id, boolean val
            queries[idx][TNVC_INSERT_BYTE] =
                db_mgr_sql_prepare(idx, "SELECT insert_tnvc_byte_entry($1::int4,NULL,NULL,$2::int2) ",
                                   "TNVC_INSERT_BYTE", 2, NULL); // tnvc_id, smallint val
            queries[idx][TNVC_INSERT_UINT] =
                db_mgr_sql_prepare(idx, "SELECT insert_tnvc_uint_entry($1::int4,NULL,NULL,$2::int4) ",
                                   "TNVC_INSERT_UINT", 2, NULL); // tnvc_id, integer val
            queries[idx][TNVC_INSERT_VAST] =
                db_mgr_sql_prepare(idx, "SELECT insert_tnvc_vast_entry($1::int4,NULL,NULL,$2::bigint) ",
                                   "TNVC_INSERT_VAST", 2, NULL); // tnvc_id, bigint val
            queries[idx][TNVC_INSERT_TV] =
                db_mgr_sql_prepare(idx, "SELECT insert_tnvc_tv_entry($1::int4,NULL,NULL,$2::int4) ", "TNVC_INSERT_TV",
                                   2, NULL); // tnvc_id, integer val
            queries[idx][TNVC_INSERT_TS] =
                db_mgr_sql_prepare(idx, "SELECT insert_tnvc_ts_entry($1::int4,NULL,NULL,$2::int4) ", "TNVC_INSERT_TS",
                                   2, NULL); // tnvc_id, integer val
            queries[idx][TNVC_INSERT_UVAST] =
                db_mgr_sql_prepare(idx, "SELECT insert_tnvc_uvast_entry($1::int4,NULL,NULL,$2::bigint) ",
                                   "TNVC_INSERT_UVAST", 2, NULL); // tnvc_id, bigint val
            queries[idx][TNVC_INSERT_REAL32] =
                db_mgr_sql_prepare(idx, "SELECT insert_tnvc_real32_entry($1::int4,NULL,NULL,$2::float8) ",
                                   "TNVC_INSERT_REAL32", 2, NULL); // tnvc_id, double val
            queries[idx][TNVC_INSERT_REAL64] =
                db_mgr_sql_prepare(idx, "SELECT insert_tnvc_real64_entry($1::int4,NULL,NULL,$2::float8) ",
                                   "TNVC_INSERT_REAL64", 2, NULL); // tnvc_id, double val

            queries[idx][TNVC_VIEW] = db_mgr_sql_prepare(
                idx, "SELECT * FROM type_name_value_collection WHERE tnvc_id = $1::int4", "TNVC_VIEW", 1, NULL);
            queries[idx][TNVC_ENTRIES] =
                db_mgr_sql_prepare(idx,
                                   "SELECT data_type_id, int_value, uint_value, obj_value, str_value, ac_value, "
                                   "tnvc_value FROM vw_tnvc_entries WHERE tnvc_id = $1::int4 ORDER BY order_num ASC",
                                   "TNVC_ENTRIES", 1, NULL);

            queries[idx][TNVC_PARMSPEC_INSERT_AC] =
                db_mgr_sql_prepare(idx, "CALL SP__insert_actual_parms_ac($1::int4,$2::int4,$3::int4)",
                                   "TNVC_PARMSPEC_INSERT_AC", 3, NULL); // ap_spec_id, order_num, ac_id
            queries[idx][TNVC_PARMSPEC_INSERT_TNVC] =
                db_mgr_sql_prepare(idx, "CALL SP__insert_actual_parms_tnvc($1::int4,$2::int4,$3::int4)",
                                   "TNVC_PARMSPEC_INSERT_TNVC", 3, NULL); // ap_spec_id, order_num, tnvc_id
            queries[idx][TNVC_PARMSPEC_CREATE] =
                db_mgr_sql_prepare(idx, "SELECT create_actual_parmspec_tnvc($1::int4,$2::int4,NULL)",
                                   "TNVC_PARMSPEC_CREATE", 2, NULL); // formal_def_id, tnvc_id, description

            queries[idx][MSGS_GET] =
                db_mgr_sql_prepare(idx, "SELECT * FROM message_group WHERE group_id=$1::int4", "MSGS_GET", 1, NULL);
            queries[idx][MSG_GET_AGENTS] =
                db_mgr_sql_prepare(idx, "SELECT agent_id_string FROM vw_message_agents WHERE message_id=$1::int4",
                                   "MSG_GET_AGENTS", 1, NULL);
            queries[idx][MSGS_GET_AGENTS] =
                db_mgr_sql_prepare(idx, "SELECT agent_id_string FROM vw_message_group_agents WHERE group_id=$1::int4",
                                   "MSGS_GET_AGENTS", 1, NULL);
            queries[idx][MSGS_ENTRIES_GET] = db_mgr_sql_prepare(
                idx, "SELECT * FROM message_group_entry WHERE group_id=$1::int4 ORDER BY order_num ASC",
                "MSGS_ENTRIES_GET", 1, NULL);
            queries[idx][MSGS_ENTRIES_GET_AGENTS] = db_mgr_sql_prepare(
                idx, "SELECT * FROM vw_message_agents WHERE message_id=$1::int4", "MSGS_ENTRIES_GET_AGENTS", 1, NULL);
            queries[idx][MSGS_UPDATE_GROUP_STATE] =
                db_mgr_sql_prepare(idx, "UPDATE message_group mg SET state_id=$1::int4 WHERE group_id=$2::int4",
                                   "MSGS_UPDATE_GROUP_STATE", 2, NULL);
            queries[idx][MSGS_OUTGOING_GET] = db_mgr_sql_prepare(
                idx, "SELECT group_id, ts FROM vw_ready_outgoing_message_groups", "MSGS_OUTGOING_GET", 0, NULL);
            queries[idx][MSGS_OUTGOING_CREATE] =
                db_mgr_sql_prepare(idx, "INSERT INTO message_group (state_id, is_outgoing) VALUES(1, TRUE)",
                                   "MSGS_OUTGOING_CREATE", 0, NULL);
            queries[idx][MSGS_INCOMING_GET] =
                db_mgr_sql_prepare(idx, "SELECT * FROM vw_ready_incoming_message_groups", "MSGS_INCOMING_GET", 0, NULL);
            queries[idx][MSGS_INCOMING_CREATE] = db_mgr_sql_prepare(
                idx, "SELECT create_incoming_message_group($1::int4, $2::varchar)", "MSGS_INCOMING_CREATE", 2,
                NULL); // Received timestamp, From Agent name (ie: ipn:2.1)
            ...
            queries[idx][DB_LOG_MSG] =
                db_mgr_sql_prepare(idx,
                                   "INSERT INTO nm_mgr_log (msg,details,level,source,file,line) "
                                   "VALUES($1::varchar,$2::text,$3::int4,$4::varchar,$5::varchar,$6::int4)",
                                   "DB_LOG_MSG", 6, NULL);
#endif // HAVE_POSTGRESQL
        }
...
// TODO postgresql : turn off autocommit
    ...
    static MYSQL_STMT *db_mgr_sql_prepare(size_t idx, const char *query)
    {
        ...
        {
            AMP_DEBUG_ERR("Failed to prepare %s: errno %d, error= %s", query, mysql_stmt_errno(rtv),
    ...
    static char *db_mgr_sql_prepare(size_t idx, const char *query, char *stmtName, int nParams, const Oid *paramTypes)
    {
        ...
        {
            AMP_DEBUG_ERR("Failed to prepare %s: errno %d, error= %s", query, PQresultStatus(pgresult),
    ...
    void db_mgr_sql_info_deserialize(blob_t * data)
    {
        ...
        QCBORDecode_Init(&it, (UsefulBufC) { data->value, data->length }, QCBOR_DECODE_MODE_NORMAL);
  ...
  //FIXME persistance
...
#endif // HAVE_POSTGRESQL
    {
        ...
        AMP_DEBUG_ENTRY("db_mgt_query_fetch", "(%p,%p)", res, format);
            ...
            */
            ...
            static int32_t db_tx_msg_group_agents(nmmgr_t * mgr, int group_id, msg_grp_t *msg_group)
            {
                ...
                int db_query_tnvc(size_t dbidx, int tnvc_id, tnvc_t *parms)
                {
                    ...
                    // Create vector
                    parms->values =
                        vec_create(array_len, tnv_cb_del, tnv_cb_comp, tnv_cb_copy, VEC_FLAG_AS_STACK, &rtv);
                    if (rtv != AMP_OK)
                    {
#ifdef HAVE_MYSQL
                        mysql_stmt_free_result(stmt);
                        ...
                        for (int i = 0; i < array_len && rtv == AMP_OK; i++)
                        {
                            ...
                            switch (tnv_type)
                            {
                                ...
                                case AMP_TYPE_TNVC:
                                    ...
                                    break;
                                default:
                                    AMP_DEBUG_ERR(__FUNCTION__, "SQL Support for TNV type %d not implemented",
                                                  tnv_type);
                                    rtv = AMP_FAIL;
                            }
                    ...
                    cace_ari_t *db_query_ari(size_t dbidx, int ari_id)
                    {
                        ...
                        DB_CHKNULL(mysql_stmt_store_result(
                            stmt)); // Results must be buffered to allow execution of nested queries
                        ...
                        {
                            strncpy(issuing_org, PQgetvalue(res, 0, issuing_org_fnum),
                                    254); // -1 to accomodate NULL-character
                            issuing_org_null = 0;
                        }
...
#endif // HAVE_MYSQL
#ifdef HAVE_POSTGRESQL
                                        PQclear(res); // TODO postgresql : unclear why this is here as the
                                                      // mysql_stmt_free_result and PQclear
#endif                                                // HAVE_POSTGRESQL
                                        ac_release(ac, 1);
                                        return NULL;
                                    }
...
// Free result now that we retrieved row
                            ...
                            int32_t db_tx_build_group(int32_t grp_idx, msg_grp_t * msg_group)
                            {
                                ...
                                AMP_DEBUG_ENTRY("db_tx_build_group", "(%d, %p)", grp_idx, msg_group);
...
/* Step 1: Find all messages for this outgoing group. */
                                ...
                                // Bind results
                                DB_CHKINT(mysql_stmt_bind_result(stmt, bind_res));
                                DB_CHKINT(mysql_stmt_store_result(
                                    stmt)); // Results must be buffered to allow execution of nested queries
#endif                                      // HAVE_MYSQL
#ifdef HAVE_POSTGRESQL
                                dbexec_prepared;
                                ...
                                while (!mysql_stmt_fetch(stmt))
                                {
                                    ...
                                    for (int i = 0; i < nrows; i++)
                                    {
                                        ...
                                        // NOTE: Support for other types can be added here in the future if needed, for
                                        ...
                                        switch (r_type_id)
                                        {
                                            case MSG_TYPE_PERF_CTRL:
                                                ...
                                                if (ctrl == NULL)
                                                {
                                                    ...
                                                    // Note: This will cause group to be marked error in DB, but we will
                                                    // still attempt to add other messages in the group (if any).
                                                    result = AMP_FAIL;
                                                }
                                                else
                                                {
                                            ...
                                            default:
                                                AMP_DEBUG_ERR(
                                                    "db_tx_build_group",
                                                    "Only Tx of Controls from the Manager is supported at this time",
                                                    NULL);
                                                continue;
                                        }
                                    }
                                ...
                                int db_tx_collect_agents(int32_t grp_idx, vector_t * vec)
                                {
                                    ...
                                    if (db_mgt_query_fetch(&res,
                                        ...
                                        != AMP_OK)
                                    {
                                        AMP_DEBUG_ERR("db_tx_collect_agents", "Can't get agents for grp: %d", grp_idx);
...
/* Step 3: For each row returned.... */
                                            ...
                                            if ((agent = db_fetch_agent(atoi(row[0]))) != NULL)
...
#endif // HAVE POSTGRESQL
                                                {
                                                    AMP_DEBUG_INFO("db_outgoing_process_recipients",
                                                                   "Adding agent name %s.", agent->eid.name);
                                ...
                                agent_t *db_fetch_agent(int32_t id)
                                {
...
#endif // HAVE_POSTGRESQL
                                    {
                                        eid_t eid;
#ifdef HAVE_MYSQL
                                        strncpy(eid.name, row[1],
                                                AMP_MAX_EID_LEN - 1); // -1 to accomodate NULL-character
#endif                                                                // HAVE_MYSQL
#ifdef HAVE_POSTGRESQL
                                        strncpy(eid.name, PQgetvalue(res, 0, name_fnum),
                                                AMP_MAX_EID_LEN - 1); // -1 to accomodate NULL-character
#endif                                                                // HAVE POSTGRESQL
...
int32_t db_fetch_protomid_idx(mid_t *mid)
{
	...
	return result;
                                    ...
                                    void db_process_outgoing(nmmgr_t * mgr)
                                    {
                                        ...
                                        int        group_id;
                                        int ts; // TODO: UVAST? TODO: change this to an output (update record) instead
                                                // of input from record
                                        dbprep_declare(DB_CTRL_CON, MSGS_OUTGOING_GET, 0,
                                                       2); // FUTURE: May add MgrEid parameter to allow a single DB to
                                        ...
                                        DB_CHKVOID(mysql_stmt_store_result(
                                            stmt)); // Results must be buffered to allow execution of nested queries
#endif                                              // HAVE_MYSQL
#ifdef HAVE_POSTGRESQL
                                        dbexec_prepared;
...
// Fetch all rows
                                        ...
                                        while (!mysql_stmt_fetch(stmt))
                                        {
                                            ...
                                            for (int i = 0; i < nrows; i++)
                                            {
                                                ...
                                                // Set timestamp
                                                msg_group->timestamp =
                                                    amp_tv_from_ctime(OS_TimeFromTotalSeconds(ts), NULL);
                                                                  ...
                                                                  uint32_t * metadata_id, uint32_t * fp_spec_id)
                                        {
                                            ...
                                            // VERIFY: Is this correct? optimal? Do we need to handle cases where name
                                            // is not numeric?
                                            ...
                                            uint32_t db_insert_ari_reg(db_con_t dbidx, cace_ari_t * ari, int *status)
                                            {
                                                ...
                                                if (ARI_GET_FLAG_NN(ari->as_reg.flags))
                                                {
                                                    ...
                                                    int adm_obj_type;
                                                    // Query metadata
                                                    if (db_query_ari_metadata(dbidx, ari, &metadata_id, &fp_spec_id)
                                                        == AMP_FAIL)
                                                    {
                                                        *status = AMP_FAIL;
                                                        DB_LOGF_ERR(dbidx, "db_insert ARI CTRL Unrecognized Nickname",
                                                                    "nn_idx=%d", ari->as_reg.nn_idx);
                                                        return 0;
                                                    }
                                                }
                                                ...
                                                {
                                                    DB_LOG_ERR(dbidx, "db_insert ARI CTRL without defined NN not "
                                                                      "currently supported");
                                                    *status = AMP_FAIL;
                                                    return 0;
                                                }
                                                ...
                                                params_id = db_insert_tnvc_params(dbidx, fp_spec_id,
                                                                                  &(ari->as_reg.parms), status);
                                                ...
                                                uint32_t db_insert_ari(db_con_t dbidx, cace_ari_t * ari, int *status)
                                                {
                                                    ...
                                                    {
                                                        case AMP_TYPE_LIT:
                                                            return db_insert_ari_lit(dbidx, ari, status);
                                                            break;
                                                            // NOTE: Custom handling (or validation) may be needed for
                                                            // some ARI Types, but reg should handle most cases
                                                        default:
                                                            return db_insert_ari_reg(dbidx, ari, status);
                                                    }
                                                }
                                                void db_insert_ac_entry(db_con_t dbidx, uint32_t ac_id, size_t idx,
                                                                        uint32_t ari_id, int *status)
                                                {
...
// In the case of an error, it will remain at the default error value of 0
                                                    ...
                                                    if (mysql_stmt_fetch(stmt) != 0)
                                                    {
                                                        AMP_DEBUG_ERR("Failed to Insert AC Entry: %s",
...
// In the case of an error, it will remain at the default error value of 0
                                                        ...
                                                        if (mysql_stmt_fetch(stmt) != 0)
                                                        {
                                                            AMP_DEBUG_ERR("Failed to Create AC: %s",
                                                                          mysql_stmt_error(stmt));
#endif // HAVE_MYSQL
#ifdef HAVE_POSTGRESQL
                                                            if (nrows == 0)
                                                        ...
                                                        void db_insert_tnv(db_con_t dbidx, uint32_t tnvc_id,
                                                                           tnv_t * tnv, int        *status)
                                                        {
                                                            dbprep_declare(dbidx, TNVC_INSERT_AC, 2,
                                                                           0); // Ignore return value, all TNVC_INSERT_*
                                                                               // primitive types have same params
                                                            ...
                                                            switch (tnv->type)
                                                            {
                                                                ...
                                                                case AMP_TYPE_STR:
                                                                    ...
                                                                    bind_param[dbidx].buffer =
                                                                        (char *)tnv->value.as_ptr;
...
#endif // HAVE_POSTGRESQL
                                                                    dbprep_bind_param_bigint(C_VAL,
                                                                                             tnv->value.as_uvast);
                                                                    break;
                                                                case AMP_TYPE_TS:
#ifdef HAVE_MYSQL
...
#endif // HAVE_POSTGRESQL
                                                                    dbprep_bind_param_double(C_VAL,
                                                                                             tnv->value.as_real64);
                                                                    break;
                                                                ...
                                                                case AMP_TYPE_AC:
                                                                    id = db_insert_ac(dbidx, (ac_t *)tnv->value.as_ptr,
                                                                                      status);
#ifdef HAVE_MYSQL
                                                                ...
                                                                default:
                                                                    DB_LOGF_ERR(
                                                                        dbidx,
                                                                        "SQL Support for TNV Type Not Implemented",
                                                                        "%d", tnv->type);
                                                                    *status = AMP_FAIL;
                                                                    return;
                                                            }
...
// In the case of an error, it will remain at the default error value of 0
                                                            ...
                                                            if (mysql_stmt_fetch(stmt) != 0)
                                                            {
                                                                AMP_DEBUG_ERR("Failed to Create TNV: %s",
                                                                              mysql_stmt_error(stmt));
#endif // HAVE_MYSQL
#ifdef HAVE_POSTGRESQL
                                                                if (dbtest_result(PGRES_TUPLES_OK) != 0)
                                                            ...
                                                            uint32_t db_insert_tnvc_params(db_con_t dbidx,
                                                                                           uint32_t fp_spec_id,
                                                                                           tnvc_t * tnvc, int *status)
                                                            {
                                                                uint32_t rtv     = 0;
       ...
       // Note: Caller determines if failure here is cause for error
                                                                ...
                                                                uint32_t db_insert_tnvc(db_con_t dbidx, tnvc_t * tnvc,
                                                                                        int     *status)
                                                                {
                                                                    CHKZERO(tnvc);
                                                                    ...
                                                                    if (num == 0)
                                                                    {
                                                                        // We won't create a tnvc if empty (0 will be
                                                                        // converted to NULL by calller)
                                                                        return 0;
                                                                    }
...
// In the case of an error, it will remain at the default error value of 0
                                                                    ...
                                                                    if (mysql_stmt_fetch(stmt) != 0)
                                                                    {
                                                                        AMP_DEBUG_ERR("Failed to Create TNVC: %s",
                                                                                      mysql_stmt_error(stmt));
#endif // HAVE_MYSQL
#ifdef HAVE_POSTGRESQL
                                                                        if (dbtest_result(PGRES_TUPLES_OK) != 0)
                                                                    ...
                                                                    void db_insert_msg_rpt_set_rpt(
                                                                        db_con_t dbidx, uint32_t entry_id, rpt_t * rpt,
                                                                        int *status)
                                                                    {
                                                                        enum cols
                                                                        {
                                                                            C_ENTRY_ID = 0,
                                                                            // C_IDX_ID, // Query set to NULL to
                                                                            // auto-insert next idx
                                                                            C_ARI_ID,
                                                                            C_PARMS_ID,
                                                                            C_TS,
                                                                            C_NUM_COLS
                                                                        };
                                                                        dbprep_declare(DB_RPT_CON,
                                                                                       MSGS_ADD_REPORT_SET_ENTRY,
                                                                                       C_NUM_COLS, 1);
                                                                        dbprep_bind_param_int(C_ENTRY_ID, entry_id);
                                                                        // adding back the AVTIME and the offset to the
                                                                        // unixposix for DB storage and display
                                                                        int64 real_time_stamp = rpt->time.ticks
                                                                                                + EPOCH_ABSTIME_DTN
                                                                                                + EPOCH_DTN_POSIX;
                                                                        ...
                                                                        uint32_t ari_id =
                                                                            db_insert_ari(dbidx, rpt->id, status);
                                                                        if (ari_id == 0)
                                                                        {
                                                                        ...
                                                                        uint32_t parms_id =
                                                                            db_insert_tnvc(dbidx, rpt->entries, status);
                                                                        if (parms_id > 0)
                                                                        { // Parameters are defined and inserted
                                                                            dbprep_bind_param_int(C_PARMS_ID, parms_id);
                                                                        }
                                                                        else
                                                                        {
                                                                            // Either status==AMP_OK and this message
                                                                            // has no parameters, or we failed to create
                                                                            // tnvc record for some reason
...
/** Insert Report **/
	...
	mysql_stmt_bind_result(stmt, bind_res);
                                                                        ...
                                                                        // Fetch results (Note: Because we are using a
                                                                        // stored procedure, we can't depend on
                                                                        // LAST_INSERT_ID) We fetch the (single) row,
                                                                        // which will automatically populate our rtv. In
                                                                        // the case of an error, it will remain at the
                                                                        // default error value of 0
#ifdef HAVE_MYSQL
                                                                        if (mysql_stmt_fetch(stmt) != 0)
                                                                        {
                                                                            AMP_DEBUG_ERR("Failed to Create Entry: %s",
                                                                                          mysql_stmt_error(stmt));
#endif // HAVE_MYSQL
#ifdef HAVE_POSTGRESQL
                                                                            if (dbtest_result(PGRES_TUPLES_OK) != 0)
                                                                        ...
                                                                        void db_insert_msg_rpt_set_name(
                                                                            db_con_t dbidx, uint32_t entry_id,
                                                                            char *name, int *status)
                                                                        {
                                                                            dbprep_declare(dbidx, MSGS_AGENT_MSG_ADD, 2,
                                                                                           0);
                                                                            ...
                                                                            DB_CHKUSR(mysql_stmt_execute(stmt), {
                                                                                *status = AMP_FAIL;
                                                                                return;
                                                                            });
#endif // HAVE_MYSQL
#ifdef HAVE_POSTGRESQL
                                                                            dbexec_prepared;
                                                                            DB_CHKUSR(dbtest_result(PGRES_COMMAND_OK), {
                                                                                *status = AMP_FAIL;
                                                                                return;
                                                                            });
                                                                            PQclear(res);
                                                                         ...
                                                                         * @param status - Set to AMP_FAIL if parsing
                                                                         * fails, but not modified on success
                                                                         * @returns Message ID, or 0 on error
                                                                         */
                                                                        uint32_t db_insert_msg_reg_agent(
                                                                            uint32_t grp_id, msg_agent_t * msg,
                                                                            int     *status)
                                                                        {
                                                                            CHKZERO(gConn[DB_RPT_CON]);
                                                                            DB_LOGF_INFO(DB_RPT_CON,
                                                                                         "Registering agent", "%s",
                                                                                         msg->agent_id.name);
                                                                            int rtv = 0;
                                                                            enum cols
                                                                            ...
                                                                            int is_ack =
                                                                                MSG_HDR_GET_ACK(msg->hdr.flags);
                                                                            int is_nak =
                                                                                MSG_HDR_GET_NACK(msg->hdr.flags);
                                                                            int is_acl =
                                                                                MSG_HDR_GET_ACL(msg->hdr.flags);
                                                                            dbprep_declare(DB_RPT_CON,
                                                                                           MSGS_REGISTER_AGENT_INSERT,
                                                                                           C_NUM_COLS, 1);
...
// In the case of an error, it will remain at the default error value of 0
                                                                            ...
                                                                            uint32_t db_insert_msg_tbl_set(
                                                                                uint32_t grp_id, msg_tbl_t * rpt,
                                                                                int     *status)
                                                                            {
                                                                                DB_LOG_ERR(DB_RPT_CON,
                                                                                           "TBL Set SQL Support TODO");
                                                                                CHKZERO(status);
                                                                                *status = AMP_FAIL;
                                                                                return 0;
                                                                            }
                                                                            /**
                                                                             * @param rpt - Report
                                                                             * @param status - Set to AMP_FAIL if
                                                                             * parsing fails, but not modified on
                                                                             * success
                                                                             * @returns Report Set ID, or 0 on error
                                                                             */
                                                                            uint32_t db_insert_msg_rpt_set(
                                                                                uint32_t grp_id, msg_rpt_t * rpt,
                                                                                int     *status)
                                                                            {
                                                                                uint32_t rtv = 0;
                                                                                vecit_t  it;
                                                                                ...
                                                                                int is_ack =
                                                                                    MSG_HDR_GET_ACK(rpt->hdr.flags);
                                                                                int is_nak =
                                                                                    MSG_HDR_GET_NACK(rpt->hdr.flags);
                                                                                int is_acl =
                                                                                    MSG_HDR_GET_ACL(rpt->hdr.flags);
...
#ifdef HAVE_MYSQL
                                                                                DB_CHKINT(mysql_stmt_bind_param(
                                                                                    stmt, bind_param));

                                                                                dbprep_bind_res_int(0, rtv);
                                                                                DB_CHKINT(mysql_stmt_bind_result(
                                                                                    stmt, bind_res));
                                                                                DB_CHKINT(mysql_stmt_execute(stmt));
#endif // HAVE_MYSQL
#ifdef HAVE_POSTGRESQL
...
// In the case of an error, it will remain at the default error value of 0
                                                                                ...
                                                                                if (dbstatus != 0)
                                                                                {
                                                                                    DB_LOGF_ERR(DB_RPT_CON,
                                                                                                ...
                                                                                                dbstatus,
                                                                                                mysql_stmt_error(stmt),
                                                                                                grp_id, is_ack, is_nak,
                                                                                                is_acl);
#endif // HAVE_MYSQL
#ifdef HAVE_POSTGRESQL
                                                                                    if (dbtest_result(PGRES_TUPLES_OK)
                                                                                        != 0)
                                                                                    {
                                                                                        DB_LOGF_ERR(
                                                                                            DB_RPT_CON,
                                                                                    ...
                                                                                    // Parse Reports
                                                                                    for (it = vecit_first(&(rpt->rpts));
                                                                                         vecit_valid(it);
                                                                                         it = vecit_next(it))
                                                                                    {
                                                                                        db_insert_msg_rpt_set_rpt(
                                                                                            DB_RPT_CON, rtv,
                                                                                            (rpt_t *)vecit_data(it),
                                                                                            status);
                                                                                    }
