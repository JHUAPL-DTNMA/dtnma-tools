...
ui_cb_return_values_t ui_print_agents_cb_fn(int idx, int keypress, void *data, char *status_msg)
{
    ...
    if (keypress == KEY_ENTER || keypress == 10)
    {
        ...
        {
            keypress =
                ui_menu_listing(agent->eid.name, agent_submenu_list, ARRAY_SIZE(agent_submenu_list), NULL, 0,
                                "F1 or 'e' to cancel. Arrow keys to navigate and enter to select. (x) indicates key "
                                "that can be pressed directly from agent listing (parent) menu to perform this action.",
                                NULL, UI_OPT_AUTO_LABEL | UI_OPT_ENTER_SEL);
    ...
    while (rtv == UI_CB_RTV_CONTINUE)
    {
        keypress = ui_menu_listing(agent->eid.name, agent_submenu_list, ARRAY_SIZE(agent_submenu_list), NULL, 0, NULL,
                                   NULL, UI_OPT_AUTO_LABEL | UI_OPT_ENTER_SEL);
        rtv      = ui_print_agents_cb_parse(idx, keypress, data, status_msg);
    }
#endif
...
int ui_print_agents()
{
    ...
    ui_menu_listing("Known Agents", list, num_agents, status_msg, 0,
                    ...
                    NULL,
#endif
                    ui_print_agents_cb_fn, UI_OPT_AUTO_LABEL);
...
cJSON *ui_json_table(tbl_t *tbl)
{
    ...
    for (int i = 0; i < num_rows; i++)
    {
        ...
        for (j = 0; j < tnvc_get_count(cur_row); j++)
        {
            ...
            cJSON_AddItemToArray(obj, ui_json_from_tnv(val));
        }
...
void ui_fprint_report(ui_print_cfg_t *fd, rpt_t *rpt)
{
    ...
    if (rpt->id->type == AMP_TYPE_RPTTPL)
    {
        ...
        {
            AMP_DEBUG_ERR("ui_print_report", "Template mismatch. Expected %d entries but have %d. Not using template.",
                          ac_get_count(&(tpl->contents)), num_entries);
            tpl = NULL;
        }
...
cJSON *ui_json_report(rpt_t *rpt)
{
    ...
    else
    {
        ...
        if (vec_num_entries(rpt->id->as_reg.parms.values) > 0)
        {
            ...
            if (obj != NULL)
            {
                cJSON_AddItemToObject(rtv, "arguments", obj);
            }
        }
    }
    time_t secs = OS_TimeGetTotalSeconds(rpt->time);
    ...
    else
    {
        ...
        if (rpt_info == NULL)
        {
            cJSON_AddItemToObject(entries, "(Entry 1)", ui_json_from_tnvc(rpt->entries));
        }
        else
        {
            cJSON_AddItemToObject(entries, rpt_info->name, ui_json_from_tnvc(rpt->entries));
        }
    }
...
char *ui_str_from_sbr(rule_t *sbr)
{
    ...
    if (str != NULL)
    {
        ...
        snprintf(str, 1023, "SBR: ID=%s, S=%" PRId64 ", E=%s, M=%" PRId64 ", C=%" PRId64 ", A=%s\n",
                 (id_str == NULL) ? "null" : id_str, OS_TimeGetTotalSeconds(sbr->start),
                 (expr_str == NULL) ? "null" : expr_str, sbr->def.as_sbr.max_eval, sbr->def.as_sbr.max_fire,
...
char *ui_str_from_tblt(tblt_t *tblt)
{
    ...
    if (tblt != NULL)
    {
        ...
        for (it = vecit_first(&(tblt->cols)); vecit_valid(it); it = vecit_next(it))
        {
            ...
            sprintf(tmp, "   %7s %15s   |", (col) ? type_to_str(col->type) : "null", (col) ? col->name : "null");
            strcat(result, tmp);
            i = 1;
        }
        strcat(result, "\n");
...
char *ui_str_from_tbr(rule_t *tbr)
{
    ...
    if (str != NULL)
    {
        ...
        snprintf(str, 1024, "TBR: ID=%s, S=" PRId64 ", P=" PRId64 ", C=" PRId64 ", A=%s\n",
                 (id_str == NULL) ? "null" : id_str, tbr->start, tbr->def.as_tbr.period, tbr->def.as_tbr.max_fire,
                 (ac_str == NULL) ? "null" : ac_str);
...
cJSON *ui_json_from_tnv(tnv_t *tnv)
{
    ...
    switch (tnv->type)
    {
            /* Primitive Types */
        case AMP_TYPE_BOOL:
            return cJSON_CreateBool(tnv->value.as_byte);
        case AMP_TYPE_INT:
            return cJSON_CreateNumber(tnv->value.as_int);
        case AMP_TYPE_UINT:
            return cJSON_CreateNumber(tnv->value.as_uint);
        case AMP_TYPE_VAST:
            return cJSON_CreateNumber(tnv->value.as_vast);
        case AMP_TYPE_TV:
        case AMP_TYPE_TS:
        case AMP_TYPE_UVAST:
            return cJSON_CreateNumber(tnv->value.as_uvast);
        case AMP_TYPE_REAL32:
            return cJSON_CreateNumber(tnv->value.as_real32);
        case AMP_TYPE_REAL64:
            return cJSON_CreateNumber(tnv->value.as_real64);

            /* Compound Objects */
        case AMP_TYPE_TNV:
            return ui_json_from_tnv(tnv->value.as_ptr);
        case AMP_TYPE_TNVC:
            return ui_json_from_tnvc(tnv->value.as_ptr);
...
char *ui_str_from_tnv(tnv_t *tnv)
{
    ...
    switch (tnv->type)
    {
        case AMP_TYPE_CNST:
        case AMP_TYPE_EDD:
            alt_str = ui_str_from_edd(tnv->value.as_ptr);
            break;
        case AMP_TYPE_CTRL:
            alt_str = ui_str_from_ctrl(tnv->value.as_ptr);
            break;
        case AMP_TYPE_LIT:
        case AMP_TYPE_ARI:
            alt_str = ui_str_from_ari(tnv->value.as_ptr, NULL, 0);
            break;
        case AMP_TYPE_MAC:
            alt_str = ui_str_from_mac(tnv->value.as_ptr);
            break;
        case AMP_TYPE_OPER:
            alt_str = ui_str_from_op(tnv->value.as_ptr);
            break;
        case AMP_TYPE_RPT:
            alt_str = ui_str_from_rpt(tnv->value.as_ptr);
            break;
        case AMP_TYPE_RPTTPL:
            alt_str = ui_str_from_rpttpl(tnv->value.as_ptr);
            break;
        case AMP_TYPE_SBR:
            alt_str = ui_str_from_sbr(tnv->value.as_ptr);
            break;
        case AMP_TYPE_TBL:
            alt_str = ui_str_from_tbl(tnv->value.as_ptr);
            break;
        case AMP_TYPE_TBLT:
            alt_str = ui_str_from_tblt(tnv->value.as_ptr);
            break;
        case AMP_TYPE_TBR:
            alt_str = ui_str_from_tbr(tnv->value.as_ptr);
            break;
        case AMP_TYPE_VAR:
            alt_str = ui_str_from_var(tnv->value.as_ptr);
            break;

        /* Primitive Types */
        case AMP_TYPE_BOOL:
        case AMP_TYPE_BYTE:
            sprintf(str, "%d", tnv->value.as_byte);
            break;
        case AMP_TYPE_STR:
            snprintf(str, 1023, "%s", (char *)tnv->value.as_ptr);
            break;
        case AMP_TYPE_INT:
            sprintf(str, "%d", tnv->value.as_int);
            break;
        case AMP_TYPE_UINT:
            sprintf(str, "%d", tnv->value.as_uint);
            break;
        case AMP_TYPE_VAST:
            sprintf(str, PRId64, tnv->value.as_vast);
            break;
        case AMP_TYPE_TV:
        case AMP_TYPE_TS:
        case AMP_TYPE_UVAST:
            sprintf(str, PRIu64, tnv->value.as_uvast);
            break;
        case AMP_TYPE_REAL32:
            sprintf(str, "%f", tnv->value.as_real32);
            break;
        case AMP_TYPE_REAL64:
            sprintf(str, "%lf", tnv->value.as_real64);
            break;

        /* Compound Objects */
        case AMP_TYPE_TNV:
            alt_str = ui_str_from_tnv(tnv->value.as_ptr);
            break;
        case AMP_TYPE_TNVC:
            alt_str = ui_str_from_tnvc(tnv->value.as_ptr);
            break;
        case AMP_TYPE_AC:
            alt_str = ui_str_from_ac(tnv->value.as_ptr);
            break;
        case AMP_TYPE_EXPR:
            alt_str = ui_str_from_expr(tnv->value.as_ptr);
            break;
        case AMP_TYPE_BYTESTR:
            alt_str = ui_str_from_blob(tnv->value.as_ptr);
            break;
