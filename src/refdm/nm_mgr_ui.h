...
/** NCURSES Style UI Helper API.  These functions will gracefully fallback to non-curses implementations if not
 * available **/
...
typedef struct form_fields_t
{
    ...
    union args
    {
        ...
        struct num
        { // Used for integer and numeric types
            int padding;
            int vmin;
            int vmax;
        } num;
...
typedef enum ui_cb_return_values_t
{
    ...
    UI_CB_RTV_STATUS   = 1,  /**< Continue displaying menu with updated status message (if ui_menu_listing was given a
                                status_msg buffer) */
