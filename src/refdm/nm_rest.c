...
/* Check if libcivetweb has been built with all required features. */
    ...
    {
        fprintf(stderr, "Error: Embedded example built with SSL support, "
...
static int agentSendRaw(struct mg_connection *conn, time_t ts, agent_t *agent, char *hex_sep)
{
    ...
    {
        mg_send_http_error(conn, HTTP_INTERNAL_ERROR, "Error creating message");
        return HTTP_INTERNAL_ERROR;
    }
    if ((msg->ac = ac_create()) == NULL)
    {
        msg_ctrl_release(msg, 1);
        mg_send_http_error(conn, HTTP_INTERNAL_ERROR, "Error creating AC");
        return HTTP_INTERNAL_ERROR;
    }
    ...
    while (part != NULL)
    {
        ...
        if (data == NULL)
        {
            mg_send_http_error(conn, HTTP_INTERNAL_ERROR, "Error creating blob from input");
            msg_ctrl_release(msg, 1);
            return HTTP_INTERNAL_ERROR;
        }
        ...
        if (id == NULL)
        {
            mg_send_http_error(conn, HTTP_INTERNAL_ERROR, "Error decoding CTRL");
            msg_ctrl_release(msg, 1);
            return HTTP_INTERNAL_ERROR;
        }
        ...
        {
            mg_send_http_error(conn, HTTP_INTERNAL_ERROR, "Error adding CTRL to message");
            ari_release(id, 1);
            msg_ctrl_release(msg, 1);
            return HTTP_INTERNAL_ERROR;
        }
...
static int agentShowTextReports(struct mg_connection *conn, agent_t *agent)
{
    ...
    start_text_page(conn);
    mg_printf(conn, "Showing %d reports for agent %s", vec_num_entries_ptr(&(agent->rpts)), agent->eid.name);
...
static int agentShowTextTables(struct mg_connection *conn, agent_t *agent)
{
    ...
    start_text_page(conn);
    mg_printf(conn, "Showing %d tables for agent %s \n", vec_num_entries_ptr(&(agent->tbls)), agent->eid.name);
 ...
 *    - PUT /agents/eid/$eid/hex/$ts - Same as above, but with Timestamp value given in URL, whereas above assumes a
 * value of 0.
...
static int agentEidHandler(struct mg_connection *conn, void *cbdata)
{
    ...
    if (cnt >= 2 && 0 == strcmp(ri->request_method, "PUT"))
    {
        ...
        if (agent == NULL)
        {
            mg_send_http_error(conn, HTTP_NOT_FOUND, "Invalid agent eid");
        ...
        else if (0 == strcmp(cmd, "hex"))
        {
            ...
            return agentSendRaw(conn, ts, agent, buffer);
        }
...
static int agentIdxHandler(struct mg_connection *conn, void *cbdata)
{
    ...
    else if (cnt >= 2 && 0 == strcmp(ri->request_method, "GET"))
    {
        if (0 == strcmp(cmd, "reports"))
        {
            if (cnt == 2 || 0 == strcmp(cmd2, "text"))
            {
                return agentShowTextReports(conn, (agent_t *)vec_at(&gMgrDB.agents, idx));
            }
            else if (cnt == 3 && 0 == strcmp(cmd2, "hex"))
            {
                return agentShowRawReports(conn, (agent_t *)vec_at(&gMgrDB.agents, idx));
            }
            else if (cnt == 3 && 0 == strcmp(cmd2, "json"))
            {
                return agentShowJSONReports(conn, (agent_t *)vec_at(&gMgrDB.agents, idx));
            }
        }
    }
