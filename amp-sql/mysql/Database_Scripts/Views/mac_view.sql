...
--
use amp_core;
CREATE OR REPLACE VIEW `vw_mac_definition` AS
    SELECT 
    ...
    FROM
        obj_metadata
            JOIN
        (SELECT 
            obj_formal_definition.obj_formal_definition_id, obj_metadata_id, use_desc, fp_spec_id, max_call_depth
        FROM
...
CREATE OR REPLACE VIEW `vw_mac_actual` AS
    SELECT 
        obj_metadata.obj_metadata_id,
        obj_name,
        namespace_id,
    ...
    FROM
        ...
        JOIN amp_core.macro_actual_definition ON obj_actual_definition.obj_actual_definition_id = macro_actual_definition.obj_actual_definition_id) AS view1 ON view1.obj_metadata_id = amp_core.obj_formal_definition.obj_metadata_id) join2 ON join2.obj_metadata_id = obj_metadata.obj_metadata_id;