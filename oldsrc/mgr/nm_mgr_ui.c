...
char *ctrl_menu_list_choices[] = {
    "List all supported ADMs.      ", "List External Data Definitions", "List Atomics (CNST, LIT)      ",
    "List Control Definitions      ", "List Macro Definitions        ", "List Operator Definitions     ",
    "List Report Templates         ", "List Rules                    ", "List Table Templates          ",
    "List Variables                ",
};
...
#if defined(HAVE_MYSQL) || defined(HAVE_POSTGRESQL)
char         *db_menu_choices[]     = { "Set Database Connection Information",
            ...
            else
            {
                switch (choice)
                {
                    case MAIN_MENU_VERSION:
                        sprintf(msg, "VERSION: Built on %s %s\nAMP Protocol Version %d - %s/%02d", __DATE__, __TIME__,
                    ...
                    case MAIN_MENU_LIST_AGENTS:
                        ...
                        break;
                    case MAIN_MENU_LIST_AMM: // List Object Information (old Control Menu merged with Admmin Menu's List
                                             // Agents)
                    ...
                    case MAIN_MENU_AUTOMATOR_UI:
                        ...
                        printf("Switching to alternate AUTOMATOR interface. Type 'EXIT_UI' to return to this menu, '?' "
                               "for usage.");
                        break;
                    default:
                        new_msg = 1;
    ...
    for (i = 0, it = vecit_first(&(col->results)); vecit_valid(it); it = vecit_next(it), i++)
    {
        ...
        if (num_parms > 0)
        {
            ...
            for (j = 0, itp = vecit_first(&(meta->parmspec)); vecit_valid(itp); itp = vecit_next(itp), j++)
            {
                ...
                sprintf((list[i].name + strlen(list[i].name)), "%s %s", type_to_str(parm->type), parm->name);
            }
            strcat(list[i].name, ")");
        }
    }

    rtv = ui_menu_listing(title, list, num_objs, NULL, 0, NULL, NULL,
...
void ui_postprocess_ctrl(cace_ari_t *id)
{
    ...
    else if ((strcmp(meta->name, AGENT_DEL_TBR_STR) == 0) || (strcmp(meta->name, AGENT_DEL_SBR_STR) == 0))
    {
        ac_t   *ac = (ac_t *)adm_get_parm_obj(&(id->as_reg.parms), 0, AMP_TYPE_AC);
        vecit_t it;
...
#ifdef USE_NCURSES
    form_fields_t fields[] = { { "EID", &line[0], AMP_MAX_EID_LEN, O_AUTOSKIP | O_NULLOK, TYPE_CHECK_REGEXP } };
    ...
    while (cace_daemon_run_get(&mgr->running))
    {
        ...
        else
        {
            switch (choice)
            {
                case 0:
                    ui_db_parms(1);
                    break; // New Connection Parameters
                case 1:
                    ui_db_parms(0);
                    break;
                case 2: // Reset Tables
                ...
                case 6:
                    ui_db_write();
                    break; // Write DB info to file.
                case 7:
                    ui_db_read();
                    break; // Read DB infor from file.
            }
        }
    }
}
...
void ui_db_parms(int do_edit)
{
    ...
    ui_form("SQL Database Connection Information",
            ((do_edit) ? "Update Connection Information" : "This form is read-only"), db_conn_form_fields, n_choices);
...
void ui_ctrl_list_menu(nmmgr_t *mgr)
{
    ...
    while (cace_daemon_run_get(&mgr->running))
    {
        ...
        else
        {
            ...
            {
                case 0:
                    ui_list_objs(ui_input_adm_id(), TYPE_MASK_ALL, NULL);
                    break;
                case 1:
                    ui_list_objs(ui_input_adm_id(), TYPE_AS_MASK(AMP_TYPE_EDD), NULL);
                    break;
                case 2:
                ...
                case 3:
                    ui_list_objs(ui_input_adm_id(), TYPE_AS_MASK(AMP_TYPE_CTRL), NULL);
                    break;

                case 4:
                    ui_list_objs(ui_input_adm_id(), TYPE_AS_MASK(AMP_TYPE_MAC), NULL);
                    break;

                case 5:
                    ui_list_objs(ui_input_adm_id(), TYPE_AS_MASK(AMP_TYPE_OPER), NULL);
                    break;

                case 6:
                    ui_list_objs(ui_input_adm_id(), TYPE_AS_MASK(AMP_TYPE_RPTTPL), NULL);
                    break;
                ...
                case 8:
                    ui_list_objs(ui_input_adm_id(), TYPE_AS_MASK(AMP_TYPE_TBLT), NULL);
                    break;

                case 9:
                    ui_list_objs(ui_input_adm_id(), TYPE_AS_MASK(AMP_TYPE_VAR), NULL);
                    break;
...
void ui_init()
{
    ...
    {
        printf("WARNING: stderr file redirection failed (%i). Warning messages may interfere with NCURSES menus",
               errno);
    }
...
int ui_display_exec()
{
    ...
    while (running)
    {
        ...
        switch (c)
        {
            ...
            default:
                // FIXME: Quit confirmation is used here primarily to hide a bug where parent menu may not refresh when
                // this pad panel is hidden
    ...
    for (i = 0; i < num_fields; i++)
    {
        ...
        switch (fields[i].type)
        {
            ...
            case TYPE_CHECK_ENUM:
                set_field_type(field[i], TYPE_ENUM, fields[i].args.en.valuelist, fields[i].args.en.checkcase,
                               fields[i].args.en.checkunique);
                break;

            case TYPE_CHECK_INT:
                set_field_type(field[i], TYPE_INTEGER, fields[i].args.num.padding, fields[i].args.num.vmin,
                               fields[i].args.num.vmax);
                break;
            case TYPE_CHECK_NUM:
                set_field_type(field[i], TYPE_NUMERIC, fields[i].args.num.padding, fields[i].args.num.vmin,
                               fields[i].args.num.vmax);
                break;
    ...
    while (running && (ch = wgetch(my_form_win)) != KEY_F(1))
    {
        ...
        switch (ch)
        {
            ...
            case KEY_ENTER: // numpad enter key
                ...
                if (field_index(current_field(my_form)) == num_fields - 1)
                {
                    ...
                    else
                    {
                        ...
                        // else fall through to F2 handling of Save/Submit (after clearing prompt, in case validation
                        // fails)
                        touchwin(my_form_win);
                        wrefresh(my_form_win);
                        refresh();
            ...
            case KEY_F(2):
                ...
                {
                    ui_update_line(my_form_win, "Field validation failed. Please correct input for current field.",
                                   LINES - 2, COLOR_PAIR(2));
                    break;
                }
                ...
                for (i = 0; i < num_fields; i++)
                {
                    if ((fields[i].opts_off & O_NULLOK) && field_status(field[i]) == FALSE)
                    {
                        ...
                        ui_update_line(my_form_win, "ERROR: This field is required", LINES - 2, COLOR_PAIR(2));
                        i = -1;
                        break;
                    }
                }
                            ...
                            switch (fields[i].type)
                            {
                                ...
                                case TYPE_CHECK_BOOL:
                                    switch (field_buffer(field[i], 0)[0])
                                    {
                                        ...
                                        case 'F':
                                        case '-':
                                        default: // For now, anything that isn't a valid truth string is considered
                                                 // false
                                            // (Alternatively, default case could fail validation ... but that seems
                                            // unnecessary for BOOL)
...
int ui_menu(char *title, char **choices, char **descriptions, int n_choices, char *msg)
{
    ...
    set_menu_sub(my_menu, derwin(my_menu_win, LINES - 4, 0, MENU_START_LINE,
                                 4 // Menu Start Column
                                 ));
...
int ui_menu_listing(char *title, ui_menu_list_t *list, int n_choices, char *status_msg, int default_idx,
                    char *usage_msg, ui_menu_listing_cb_fn fn, int flags)
{
            ...
            default:
                ...
                else if (fn != NULL)
                {
                    ...
                    else if (status == UI_CB_RTV_STATUS)
                    {
                        ...
                        if (status_msg != NULL)
                        {
                            ui_update_line(my_menu_win, status_msg, LINES - 2, COLOR_PAIR(2));
                        }
                    }
...
int ui_menu_select(char *title, const char *const *choices, const char *const *descriptions, int n_choices, char *msg,
                   int menu_cols)
{
    WINDOW *my_menu_win;
    PANEL  *my_pan;
...
static int do_ui_form(char *title, char *msg, form_fields_t *fields, int num_fields)
{
    ...
    for (i = 0; i < num_fields && status; i++)
    {
        ...
        // TODO: Update prompt
        printf(KGRN "%s (field %i of %i)\n" RST "-----\n%s: ", title, i + 1, num_fields, field->title);
        ui_form_show_value(field);
        printf("\n");
