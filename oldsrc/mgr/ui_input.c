...
char *ui_input_string(const char *prompt)
{
    ...
    char         *result;
    form_fields_t fields[] = { { "String", line, MAX_INPUT_BYTES, 0, 0 } };
...
cace_ari_t *ui_input_ari(const char *prompt, uint8_t adm_id, amp_uvast mask)
{
    ...
    if ((result->type != AMP_TYPE_LIT) && (result->type != AMP_TYPE_OPER) && (ARI_GET_FLAG_PARM(result->as_reg.flags)))
    {
...
int ui_input_ari_flags(uint8_t *flag)
{
    ...
    *flag = 0;
    ...
    if (type != AMP_TYPE_LIT)
    {
        ...
        form_fields_t fields[] = { { "Nicename Present?", (char *)&nn, sizeof(int), 0, TYPE_CHECK_INT },
        ...
        fields[0].args.num.vmax    = 1;
        fields[1].args.num.padding = 0;
        fields[1].args.num.vmin    = 0;
        fields[1].args.num.vmax    = 1;
        fields[2].args.num.padding = 0;
        fields[2].args.num.vmin    = 0;
        fields[2].args.num.vmax    = 1;
        fields[3].args.num.padding = 0;
        fields[3].args.num.vmin    = 0;
        fields[3].args.num.vmax    = 1;

        ui_form("Build an ARI Flag Byte", "Enter 1 for yes, 0 for no.  Enter or arrow keys to advance fields", fields,
                ARRAY_SIZE(fields));
        if (atoi(nn) != 0)
        {
...
cace_ari_t *ui_input_ari_lit(const char *prompt)
{
    ...
    mask = TYPE_AS_MASK(AMP_TYPE_INT) | TYPE_AS_MASK(AMP_TYPE_INT) | TYPE_AS_MASK(AMP_TYPE_UINT)
           | TYPE_AS_MASK(AMP_TYPE_VAST) | TYPE_AS_MASK(AMP_TYPE_UVAST) | TYPE_AS_MASK(AMP_TYPE_REAL32)
           | TYPE_AS_MASK(AMP_TYPE_REAL64);
...
tnv_t *ui_input_tnv(int type, char *prompt)
{
    ...
    switch (type)
    {
        ...
        case AMP_TYPE_REAL64:
            ...
            break;
        case AMP_TYPE_STR:
            result = tnv_from_obj(type, ui_input_string(prompt));
            break;
        case AMP_TYPE_BYTESTR:
            result = tnv_from_obj(type, ui_input_blob(prompt, 0));
            break;
        case AMP_TYPE_CNST:
        case AMP_TYPE_EDD:
        case AMP_TYPE_ARI:
            result = tnv_from_obj(type, ui_input_ari(prompt, ADM_ENUM_ALL, TYPE_MASK_ALL));
            break;
        case AMP_TYPE_MAC:
            result = tnv_from_obj(type, ui_input_mac(prompt));
            break;
        case AMP_TYPE_AC:
            result = tnv_from_obj(type, ui_input_ac(prompt));
            break;
        case AMP_TYPE_TNVC:
            result = tnv_from_obj(type, ui_input_tnvc(prompt));
            break;
        case AMP_TYPE_CTRL:
            result = tnv_from_obj(type, ui_input_ctrl(prompt));
            break;
        case AMP_TYPE_EXPR:
            result = tnv_from_obj(type, ui_input_expr(prompt));
            break;
        case AMP_TYPE_OPER:
            result = tnv_from_obj(type, ui_input_oper(prompt));
            break;
        case AMP_TYPE_RPTTPL:
            result = tnv_from_obj(type, ui_input_rpttpl(prompt));
            break;
        case AMP_TYPE_TBR:
        case AMP_TYPE_SBR:
            result = tnv_from_obj(type, ui_input_rule(prompt));
            break;
        case AMP_TYPE_TBLT:
            result = tnv_from_obj(type, ui_input_tblt(prompt));
            break;
        case AMP_TYPE_VAR:
            result = tnv_from_obj(type, ui_input_var(prompt));
            break;
        case AMP_TYPE_LIT:
            result = tnv_from_obj(type, ui_input_ari_lit(prompt));
            break;
        default:
            break;
    }
