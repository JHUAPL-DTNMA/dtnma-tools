...
static void check_produce(ari_t *value, cace_amm_const_desc_t *obj, const char *valhex, const char *refhex,
                          const char *outhex, int expect_res)
{
    ari_t inval = ARI_INIT_UNDEFINED;
    ari_convert(&inval, valhex);

    ari_t inref = ARI_INIT_UNDEFINED;
    ari_convert(&inref, refhex);
    TEST_ASSERT_TRUE_MESSAGE(inref.is_ref, "invalid reference");

    ari_set_copy(&(obj->value), &inval);

    cace_amm_valprod_ctx_t ctx;

    int res = cace_amm_valprod_ctx_init(&ctx, obj->base.fparams, &inref);
    TEST_ASSERT_EQUAL_INT_MESSAGE(expect_res, res, "cace_amm_valprod_ctx_init() disagrees");

    res = cace_amm_const_desc_produce(obj, &ctx);
...
void test_const_produce_param_none(const char *valhex, const char *refhex, const char *outhex, int expect_res)
{
    ...
    check_produce(&value, &obj, valhex, refhex, outhex, expect_res);
