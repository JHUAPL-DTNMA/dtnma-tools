...
double * wscbor_require_float64(wmem_allocator_t *alloc, wscbor_chunk_t *chunk) {
    if (!wscbor_require_major_type(chunk, CBOR_TYPE_FLOAT_CTRL)) {
        return NULL;
    }
    size_t fsize;
    ...
    if (fsize == 0) {
        wmem_list_append(chunk->errors, wscbor_error_new(
            chunk->_priv->alloc, &ei_cbor_wrong_type,
            "Item is not a float minor type: %d",
            chunk->type_minor
        ));
        return NULL;
    }

    double *val = wmem_new(alloc, double);
    switch (fsize) {
    case 2: {
        // Algorithm from Appendix D of RFC 8949
        const guint16 rawvalue = GUINT16_TO_BE(chunk->head_value);
        const unsigned char *halfp = (const unsigned char *)&rawvalue;
...
proto_item * proto_tree_add_cbor_float64(proto_tree *tree, int hfindex, packet_info *pinfo, tvbuff_t *tvb, const wscbor_chunk_t *chunk, const double *value) {
    proto_item *item = proto_tree_add_double(tree, hfindex, tvb, chunk->start, chunk->head_length, value ? *value : NAN);
    wscbor_chunk_mark_errors(pinfo, item, chunk);
    return item;
}
